...
function GdbCompareChartGDB({onTFChange, liveGSB, liveGDBS, liveBench, liveGC}){
  ...
  // CGIC/CGIS : courbe complète depuis 2022 via composition des rendements mensuels
  // (GC_FULL/GSB ne remontent qu'à ~2024). Le graphe rebase en base 100 ensuite.
  const _monthlyIndex = (MAP) => {
    ...
    yrs.forEach(y=>{ const d=MAP[y]; if(!d||!d.m) return;
      d.m.forEach((ml,i)=>{ if(!ml) return;
        ...
        idx=idx*(1+ret);
        out[y+"-"+String(i+1).padStart(2,"0")]=idx;
      });
    });
    return out;
  };
  const _cgicIdx=_monthlyIndex(typeof CRYPTO_MONTHLY!=="undefined"?CRYPTO_MONTHLY:{});
  const _cgisIdx=_monthlyIndex(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:{});
  const gcRaw  = dates.map(d=>{ const v=_cgicIdx[d.slice(0,7)]; return v!=null?v:null; });
  const gsRaw  = dates.map(d=>{ const v=_cgisIdx[d.slice(0,7)]; return v!=null?v:null; });
