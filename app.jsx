...
var CGI_PIN_TTL = 10*60*1000;   // #76/#80 — déverrouillage valable 10 min (confort PC) ; redemandé à chaque vrai chargement et au retour après expiration
...
function App(){
  ...
  React.useEffect(()=>{
    function _relock(){ try{ if(cgiGetPin() && !cgiUnlockValid()){ cgiLockClear(); setUnlocked(false); setDemo(false); setDemoMode(null); } }catch(e){} }
    ...
    // Filet de sécurité iOS PWA : certains retours d'arrière-plan ne déclenchent aucun événement
    // → vérification périodique (le TTL borne la fenêtre d'exposition).
    const _iv=setInterval(_relock, 5000);
    return ()=>{ document.removeEventListener("visibilitychange", _relock); window.removeEventListener("pageshow", _relock); window.removeEventListener("focus", _relock); clearInterval(_iv); };
  }, []);
          ...
          try{
            ...
            try{
              if(kvBank && kvBank.savedAt){
                ...
                if(kvBank.savedAt>_lbTs){ lsv9Set('cgi_bank', kvBank);
                  console.info("[bank] cgi_bank adopté du cloud (snapshot "+new Date(kvBank.savedAt).toISOString().slice(0,16)+")"); }
              }
            }catch(e){}
