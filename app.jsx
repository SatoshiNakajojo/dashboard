...
function App(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        // #90 — PRÉSERVER le savedAt local (jamais Date.now()) : les vraies éditions passent désormais
        // par __cgiSetMatelas (qui force un savedAt gagnant). Cet observateur ne doit donc JAMAIS
        // fabriquer un horodatage frais, sinon il ré-écrivait l'ancien solde du PC par-dessus l'édition
        // fraîche d'un autre appareil (clobber). Avec un savedAt préservé, le garde-fou du worker le
        // rejette si une édition plus récente existe.
        const _bkSA=(function(){ try{ const b=lsv9Get('cgi_bank'); return (b&&b.savedAt)||0; }catch(e){ return 0; } })();
        const ok=await saveBase('cgi_bank', {...live.bank, breakdown:{...bd}, totalEUR:total, ...(_bkSA?{savedAt:_bkSA}:{})});
        console.info("[bank] solde local synchronisé "+(ok?"(push OK, savedAt préservé)":"(push échoué)")+" ("+sig+")");
  ...
  const doSnapUpload = useCallback(async()=>{
    ...
    try {
      ...
      saveBase('cgi_bank',      bankSnapshotSafe(_srcPos.bank || CURRENT.bank)); // #90 — savedAt préservé, jamais Date.now()
    ...
    for(let attempt = 1; attempt <= 3 && !basesOk; attempt++){
      try {
        const bases = {
          ...
          cgi_bank:      bankSnapshotSafe((EFF||CURRENT).bank || CURRENT.bank), // #90 — savedAt préservé, jamais Date.now()
  ...
  useEffect(()=>{
    ...
    try {
      ...
      if(live.bank)      saveBase('cgi_bank',      bankSnapshotSafe(live.bank)); // #90 — un trade ne modifie pas le solde matelas : savedAt préservé, pas de re-tampon
