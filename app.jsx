...
function cgiAudit(){
  ...
  try{ console.log("%c=== JCGI · TRONC DE VÉRITÉ ===","font-weight:bold;color:#C9A86A"); }catch(e){}
  ...
  return { ref:ref, sources:CGI_TRUTH.sources, ecarts:ecarts };
}
  ...
  const handleRefresh = useCallback(async()=>{
    ...
    try{
      ...
      setLive(prev2=>{
        ...
        // #fund-pl — handleRefresh écrit total/crypto/actions (fusion ; les P&L fonds sont écrits par l'onglet JCGI)
        try{
          if(prev2 && prev2.crypto && prev2.stocks){
            ...
            saveBase('cgi_fund_stats',{..._cur,
              ...
              usdEur:_ue3, ts:Date.now()});
            cgiReport("Baromètre · snapshot", {totalUSD:Math.round(prev2.totalUSD||0), cryptoUSD:Math.round(prev2.crypto.total||0), stocksUSD:Math.round(prev2.stocks.total||0)});
          }
        }catch(e){}
        return prev2;
      });
