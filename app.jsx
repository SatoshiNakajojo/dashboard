...
function App(){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try {
        ...
        if(res.ok){
          ...
          try{
            ...
            try{ if(kvData.cgi_ibkr_truth && kvData.cgi_ibkr_truth.ts) lsv9Set('cgi_ibkr_truth', kvData.cgi_ibkr_truth); }catch(e){}
          }catch(e){}
      ...
      setLiveGDBS(cleanedGDBS);
          ...
          try{ const _it=kvData.cgi_ibkr_trades; if(_it && Array.isArray(_it.trades)) setIbkrTrades(_it.trades); }catch(e){}
  ...
  const applyPositionsFromTxns=useCallback(()=>{
    ...
    if(base.portfolio && base.portfolio.items && base.portfolio.items.length>0){
      ...
      try{ const _tr=lsv9Get('cgi_ibkr_truth');
        if(_tr&&_tr.ts&&(Date.now()-_tr.ts)<48*36e5&&_tr.cashUSD!=null) DIP_USD=_tr.cashUSD; }catch(e){}
    ...
    try{ const _trb=lsv9Get('cgi_ibkr_truth');   // #67n — clé de vérité IBKR prioritaire
