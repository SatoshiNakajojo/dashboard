...
function PageOverview({chartData,onSnapshot,eur,setEur,hidden,setHidden,EFF,refreshing,handleRefresh,refreshedAt,refreshErr,fromSnapshot,gistSync,liveDD,liveCM,liveGDBS,liveGC,chosenSource,iconDbVersion=0,bumpIconDb}){
  ...
  // #75 — money-weighted (net des apports), cohérent avec « depuis création » des fonds.
  // L'ancien pct = e/s−1 incluait les capitaux versés → +1116 % aberrant sur « tout l'historique ».
  const _blendPct = (_heroRow.total.perf!=null) ? _heroRow.total.perf : ((_heroRow.total.pct!=null)?_heroRow.total.pct:0);
  ...
  const _blendPnl = eur ? _netPnlEUR : Math.round(_netPnlEUR/_ue);
          ...
          {SECTIONS.map(sec=>(
            <SectionRow
              ...
              onTickerClick={(t, cat, item)=>{
                ...
                const NO_MODAL=["BCI","Bourso","DeBlock","KUCOIN"];
...
function GdbCompareChartGDB({tf:tfProp, onTFChange, liveGSB, liveGDBS, liveBench, liveGC, liveCgicIdx, liveCgisIdx}){
  ...
  // #77 — anti-pic LOCAL : un point qui s'écarte de >18 % de ses deux voisins immédiats
  // ALORS QUE ces voisins concordent entre eux (<8 %) est une aberration ponctuelle
  // (pic vertical isolé). On le neutralise → la courbe interpole proprement au travers.
  const despike = (vals) => {
    const out = vals.slice();
    for(let i=1;i<out.length-1;i++){
      const a=out[i-1], b=out[i], c=out[i+1];
      if(a==null||b==null||c==null||a<=0||c<=0) continue;
      const neighboursAgree = Math.abs(c-a)/((a+c)/2) < 0.08;
      const dev = Math.min(Math.abs(b-a)/a, Math.abs(b-c)/c);
      if(neighboursAgree && dev > 0.18) out[i]=null;
    }
    return out;
  };
  const gcB  = rebase(despike(sanitize(gcRaw)));
  const gsB  = rebase(despike(sanitize(gsRaw)));
...
var CGI_PIN_TTL = 90*1000;   // #76 — déverrouillage valable 90 s (anti double-montage React), plus de session persistante
...
function App(){
  ...
  React.useEffect(()=>{
    function _relock(){ try{ if(document.visibilityState==="visible" && cgiGetPin() && !cgiUnlockValid()){ cgiLockClear(); setUnlocked(false); setDemo(false); } }catch(e){} }
    document.addEventListener("visibilitychange", _relock);
    window.addEventListener("pageshow", _relock);
    window.addEventListener("focus", _relock);
    return ()=>{ document.removeEventListener("visibilitychange", _relock); window.removeEventListener("pageshow", _relock); window.removeEventListener("focus", _relock); };
  }, []);
