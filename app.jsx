...
function GdbCompareChartGDB({tf:tfProp, onTFChange, liveGSB, liveGDBS, liveBench, liveGC, liveCgicIdx, liveCgisIdx}){
  ...
  const _cgicA=gdbFundAnchors(typeof CRYPTO_MONTHLY!=="undefined"?CRYPTO_MONTHLY:{});
  const _cgisA=gdbFundAnchors(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:{});
  // #fix — point live AUJOURD'HUI pour que les courbes atteignent la valeur actuelle (sinon gelées au dernier mois clôturé)
  const _todayNC = (typeof todayNC==="function") ? todayNC() : new Date().toISOString().slice(0,10);
  if(liveCgicIdx!=null && liveCgicIdx>0) _cgicA.push({d:_todayNC, v:liveCgicIdx});
  if(liveCgisIdx!=null && liveCgisIdx>0) _cgisA.push({d:_todayNC, v:liveCgisIdx});
...
{chartData,hidden,EFF,eur,liveGSB,liveGDBS,liveBench,liveGC,liveDD,liveInv}){
  ...
  // Sparkline : quotidien (live, granulaire) si dispo, sinon série mensuelle prolongée jusqu'au live.
  const _cgicSpark = _fundDaily(2, TF[benchTF]) || gdbFundSeries(_cgicAnchors, TF[benchTF], _liveCgicIdx);
  const _cgisSpark = _fundDaily(1, TF[benchTF]) || gdbFundSeries(_cgisAnchors, TF[benchTF], _liveCgisIdx);
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:8,marginBottom:4}}>
        <FondCard label="CGIC — Crypto" cours={gcNAV} qty={_ncgic.sh} fonds={gcFonds} color={C.btc} hidden={hidden}
          ...
          tfLabel={benchTF} perf={gcPerf(TF[benchTF])} spark={_cgicSpark}/>
        <FondCard label="CGIS — Actions" cours={gsNAV} qty={_ncgis.sh} fonds={gsFonds} color={C.blue} hidden={hidden}
          ...
          tfLabel={benchTF} perf={gsPerf(TF[benchTF])} spark={_cgisSpark}/>
      </div>
