  ...
  React.useEffect(()=>{
    try{
      ...
      saveBase('cgi_fund_stats',{..._cur,
        cgic:{nav:gcNAV*_ue, valueEUR:Math.round(gcFonds*_ue), valueUSD:Math.round(gcFonds), pnlPct:gcPerfAllTime*100, sh:_ncgic.sh, mEUR:_ncgic.m},
        cgis:{nav:gsNAV*_ue, valueEUR:Math.round(gsFonds*_ue), valueUSD:Math.round(gsFonds), pnlPct:gsPerfAllTime*100, sh:_ncgis.sh, mEUR:_ncgis.m},
        ...
        if(res.ok){
          ...
          try{
            ...
            if(!_txnsMig){
              ...
              const merged = unionTxnsById(_localTx, _kvTx);
              if(_purgedRaw>0 || merged.length !== _localTx.length || merged.length !== _kvTx.length){ setTxns(merged); lsv9Set('cgi_txns', merged); }
