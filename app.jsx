...
var cgiSound = (function(){
  ...
  function note(freq,t0,dur,type,peak){
    ...
    g.gain.exponentialRampToValueAtTime(peak||0.05, t+0.012);
  ...
  function play(seq){
    ...
    seq.forEach(function(n){ note(n[0],n[1],n[2],n[3],n[4]); });
  }
  return {
    isOn:function(){ return enabled; },
    setOn:function(v){ enabled=!!v; try{ localStorage.setItem("cgi_sound", v?"1":"0"); }catch(e){} if(v) this.click(); },
    toggle:function(){ this.setOn(!enabled); return enabled; },
    click:function(){ play([[1318.5,0,0.07,"sine",0.045]]); },                                  // tick léger (E6)
    back:function(){ play([[880,0,0.10,"sine",0.05],[587.33,0.05,0.13,"sine",0.045]]); },         // descendant
    home:function(){ play([[659.25,0,0.10,"triangle",0.05],[987.77,0.07,0.15,"triangle",0.045]]); }, // montant (E5→B5)
    transaction:function(){ play([[523.25,0,0.12,"triangle",0.05],[659.25,0.085,0.12,"triangle",0.05],[783.99,0.17,0.22,"triangle",0.05]]); }, // accord C-E-G
  };
})();
...
function PageOverview({chartData,onSnapshot,eur,setEur,hidden,setHidden,EFF,refreshing,handleRefresh,refreshedAt,refreshErr,fromSnapshot,gistSync,liveDD,liveCM,liveGDBS,liveGC,chosenSource,iconDbVersion=0,bumpIconDb}){
  ...
  const _blendPct = (_heroRow.total.pct!=null) ? _heroRow.total.pct : 0;
  const _bUp = _blendPct>=0;
  const _blendCur = eur ? _sumEUR : _sumUSD;
  const _blendPnl = eur ? (_heroRow.total.pnl||0) : Math.round((_heroRow.total.pnl||0)/_ue);
  ...
  React.useEffect(()=>{
    try{
      ...
      saveBase('cgi_fund_stats', Object.assign({}, _cur, {
        ...
        cgic:{ nav:(FUND_PARTS.C>0? cEUR/FUND_PARTS.C : 0), valueEUR:cEUR, pnlPct:(_allRow.crypto.perf!=null?_allRow.crypto.perf*100:0) },
        cgis:{ nav:(FUND_PARTS.S>0? sEUR/FUND_PARTS.S : 0), valueEUR:sEUR, pnlPct:(_allRow.stocks.perf!=null?_allRow.stocks.perf*100:0) },
        ts:Date.now(), tsFunds:Date.now()
      }));
    }catch(e){}
...
function App(){
  ...
  const[unlocked,setUnlocked]=useState(()=>{ try{ return !cgiGetPin() || cgiUnlockValid(); }catch(e){ return true; } });
          ...
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              <button onClick={()=>{const v=cgiSound.toggle();setSoundOn(v);}} data-snd="none" style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",padding:"12px 14px",borderRadius:10,border:`1px solid ${C.border}`,background:C.bg,cursor:"pointer"}}>
                <span style={{display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:18}}>{soundOn?"🔊":"🔈"}</span><span style={{fontSize:13,fontWeight:700,color:C.text}}>Sons de navigation</span></span>
        ...
        <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:C.bg1,borderRadius:"20px 20px 0 0",padding:"20px 20px 36px",width:"100%",maxWidth:430,maxHeight:"85vh",overflowY:"auto",border:`1px solid ${C.border}`}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {[
                ["v6.91","Sons de navigation (#71) : retours sonores courts et discrets synthétisés (clic léger sur les boutons, accord à la validation d'une transaction, montée vers l'Accueil, descente au retour). Activables/coupables dans Réglages → « Sons de navigation ». Baromètre Telegram (#2) retravaillé côté serveur : « Allocation du fonds » mise en avant, actifs sous gestion en pied discret, titre allégé, et P&L crypto « depuis l'origine » désormais issu uniquement de la source de vérité (fini l'approximation €/$)."],
