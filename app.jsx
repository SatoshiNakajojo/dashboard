      ...
      try {
        ...
        if(res.ok){
          ...
          try{
            ...
            var _SEEDVER_KEY="cgi_txns_seedver";  // clé locale dédiée (lsv9Set rejette les clés hors LSV9_KEYS)
            var _txnsMig = false;
            try{ _txnsMig = (localStorage.getItem(_SEEDVER_KEY)===TXNS_SEEDVER); }catch(e){}
            if(!_txnsMig){
              var _existingTx = null; try{ _existingTx = lsv9Get('cgi_txns'); }catch(e){}
              if(Array.isArray(_existingTx) && _existingTx.length>0){
                // Des transactions sont déjà présentes (imports, autre appareil) → NE PAS écraser.
                // On marque simplement la migration comme faite pour ne plus jamais ré-injecter le seed.
                try{ localStorage.setItem(_SEEDVER_KEY, TXNS_SEEDVER); }catch(e){}
                const kvTx0 = Array.isArray(kvData.cgi_txns) ? kvData.cgi_txns : [];
                const merged0 = unionTxnsById(tx, kvTx0);
                if(merged0.length !== tx.length){ setTxns(merged0); lsv9Set('cgi_txns', merged0); }
                if(merged0.length > kvTx0.length){ saveBase('cgi_txns', merged0); }
                console.info("[txns] migration marquée faite (transactions existantes préservées: "+_existingTx.length+").");
              } else {
                setTxns(SEED_TXNS_REAL);
                lsv9Set('cgi_txns', SEED_TXNS_REAL);
                try{ localStorage.setItem(_SEEDVER_KEY, TXNS_SEEDVER); }catch(e){}
                saveBase('cgi_txns', SEED_TXNS_REAL);
                console.info("[txns] seed initial: "+SEED_TXNS_REAL.length+" transactions.");
              }
            } else {
