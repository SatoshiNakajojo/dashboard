  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:80,background:"rgba(0,0,0,.72)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:560,maxHeight:"82vh",overflowY:"auto",background:C.bg2,border:`1px solid ${C.border2}`,borderRadius:"16px 16px 0 0",padding:"16px 16px 28px"}}>
        ...
        {/* #90 — éditeur tactile du solde */}
        <div style={{marginTop:14,paddingTop:12,borderTop:`1px solid ${C.border2}`}}>
          {!edit ? (
            <button onClick={()=>{ setMsg(null); setEdit(true); }} style={{width:"100%",padding:"11px 12px",background:C.bg,border:`1px solid ${C.border2}`,borderRadius:10,color:C.text,fontSize:13,fontWeight:600,cursor:"pointer"}}>Modifier le solde</button>
          ) : (
            <div>
              <div style={{fontSize:11,color:C.text3,marginBottom:6}}>Nouveau solde {label||bank} (en €)</div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input type="number" inputMode="numeric" value={amt} onChange={e=>setAmt(e.target.value)} disabled={busy}
                  style={{flex:1,padding:"11px 12px",background:C.bg,border:`1px solid ${C.border2}`,borderRadius:10,color:C.text,fontSize:16,fontVariantNumeric:"tabular-nums",outline:"none"}} />
          ...
          {msg && <div style={{marginTop:8,fontSize:11.5,lineHeight:1.4,color:msg.t==="ok"?C.green:(msg.t==="err"?C.red:C.text3)}}>{msg.x}</div>}
        </div>
      </div>
    </div>
  );
