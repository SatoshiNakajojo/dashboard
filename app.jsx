...
// Prolonge des ancres jusqu'à aujourd'hui via la FORME quotidienne du KV (col 1=CGIS, 2=CGIC),
// ré-ancrée log-linéairement entre la dernière ancre (fin mai) et liveIdx (valeur live d'aujourd'hui).
// → endpoints exacts (cohérence garantie), détail quotidien de juin emprunté au KV.
function gdbAppendJune(anchors, navCol, liveIdx){
  try{
    const KV = (typeof KV_JUNE!=="undefined"?KV_JUNE:[]);
    if(!anchors||!anchors.length) return anchors;
    const last = anchors[anchors.length-1];
    const today = _todayNCstr();
    const kv = KV.filter(r=>r&&r[0]>last.d && r[0]<=today && r[navCol]!=null && r[navCol]>0);
    const target = (liveIdx!=null&&liveIdx>0) ? liveIdx : last.v;
    if(!kv.length){ if(last.d<today) anchors.push({d:today, v:target}); return anchors; }
    const n=kv.length, nav0=kv[0][navCol], navN=kv[n-1][navCol];
    // Forme quotidienne = ratios KV depuis le 1er point (préserve les vrais mouvements), puis on
    // répartit géométriquement une légère correction pour que le DERNIER point atteigne la valeur live.
    const provN = last.v * (navN/nav0);
    const k = (provN>0) ? (target/provN) : 1;
    kv.forEach((row,i)=>{
      const prog = (n>1) ? i/(n-1) : 1;
      anchors.push({d:row[0], v: last.v * (row[navCol]/nav0) * Math.pow(k, prog)});
    });
    if(kv[n-1][0]<today) anchors.push({d:today, v:target});
    return anchors;
...
function GdbCompareChartGDB({tf:tfProp, onTFChange, liveGSB, liveGDBS, liveBench, liveGC, liveCgicIdx, liveCgisIdx}){
  ...
  const _cgicA=gdbAppendJune(gdbCgicDailyAnchors(), 2, liveCgicIdx);
  const _cgisA=gdbAppendJune(gdbFundAnchors(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:{}), 1, liveCgisIdx);
...
{chartData,hidden,EFF,eur,liveGSB,liveGDBS,liveBench,liveGC,liveDD,liveInv}){
  ...
  const _cgicAnchors = gdbAppendJune(gdbCgicDailyAnchors(), 2, _liveCgicIdx);
  const _cgisAnchors = gdbAppendJune(gdbFundAnchors(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:{}), 1, _liveCgisIdx);
  // La sparkline ET le P&L de la carte dérivent de LA MÊME série (cohérence garantie).
  const _cgicSpark = gdbFundSeries(_cgicAnchors, TF[benchTF], _liveCgicIdx);
  const _cgisSpark = gdbFundSeries(_cgisAnchors, TF[benchTF], _liveCgisIdx);
