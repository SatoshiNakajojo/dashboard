  ...
  const handleRefresh = useCallback(async()=>{
    ...
    try{
      ...
      setLive(prev2=>{
        ...
        // #fund-pl — P&L EXACT = money-weighted depuis INV_SEED (identique onglet JCGI, l.5397)
        try{
          if(prev2 && prev2.crypto && prev2.stocks){
            const _ue3 = prev2.usdEur||CURRENT.usdEur;
            const _gp3 = calcGdbPrices(prev2);
            // Source des mouvements = cgi_inv LIVE (à jour), sinon _INV/seed (sinon CGIS récent est faux)
            let _invG=INV_SEED_OK;
            try{ const _li=lsv9Get('cgi_inv'); if(Array.isArray(_li)&&_li.length) _invG=_li; else if(typeof _INV!=="undefined"&&_INV&&_INV.length) _invG=_INV; }catch(e){ if(typeof _INV!=="undefined"&&_INV) _invG=_INV; }
            const _nf = f => { let m=0; _invG.forEach(x=>{ if(x&&x.fonds===f) m+=(x.io==="OUT"?-1:1)*(x.montant||0); }); return m; };
            const _nfs = f => { let sh=0; _invG.forEach(x=>{ if(x&&x.fonds===f) sh+=(x.io==="OUT"?-1:1)*(x.shares||0); }); return sh; };
            const _mC=_nf("CGIC"), _mS=_nf("CGIS"), _shC=_nfs("CGIC"), _shS=_nfs("CGIS");
            const gcF=(_gp3.gdbCfondsUSD!=null?_gp3.gdbCfondsUSD:(prev2.crypto?prev2.crypto.total:0));
            let gsF=_gp3.gdbSfondsUSD;
            if(gsF==null || !(gsF>0)){   // même fallback que l'onglet JCGI (l.5387)
              try{ const _si=(prev2.stocks&&prev2.stocks.items)||[];
                gsF=_si.filter(x=>x.cat!=="Cash").reduce((s,x)=>s+(x.val||0),0)+(((_si.find(x=>x.t==="EURO"))||{}).val||0);
              }catch(e){ gsF=0; }
            }
            const pC=(_mC>0 && isFinite(gcF)) ? (gcF*_ue3)/_mC - 1 : null;
            const pS=(_mS>0 && isFinite(gsF)) ? (gsF*_ue3)/_mS - 1 : null;
            const navC=(_shC>0 && isFinite(gcF)) ? (gcF/_shC)*_ue3 : null;
            const navS=(_shS>0 && isFinite(gsF)) ? (gsF/_shS)*_ue3 : null;
            saveBase('cgi_fund_stats',{
              totalUSD:Math.round(prev2.totalUSD||0),
              totalEUR:Math.round((prev2.totalUSD||0)*_ue3),
              cryptoUSD:Math.round(prev2.crypto.total||0),
              stocksUSD:Math.round(prev2.stocks.total||0),
              usdEur:_ue3,
              cgic:{nav:navC,valueEUR:Math.round(gcF*_ue3),pnlPct:pC!=null?pC*100:null},
              cgis:{nav:navS,valueEUR:Math.round(gsF*_ue3),pnlPct:pS!=null?pS*100:null},
              _diag:{mC:Math.round(_mC),mS:Math.round(_mS),shC:Math.round(_shC),shS:Math.round(_shS),gcF:Math.round(gcF),gsF:Math.round(gsF),ue:_ue3,invLen:(_invG||[]).length},
              ts:Date.now()
            });
          }
        }catch(e){}
