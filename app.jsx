...
function PageOverview({chartData,onSnapshot,eur,setEur,hidden,setHidden,EFF,refreshing,handleRefresh,refreshedAt,refreshErr,fromSnapshot,gistSync,liveDD,liveCM,liveGDBS,liveGC,chosenSource,iconDbVersion=0,bumpIconDb}){
  ...
  const _fundSparkVals = (fond, valEUR) => {
    ...
    let A = (fond==="CGIC"?_cmAnch:_smAnch).filter(p=>p.d<=today); // exclut les mois futurs (plats)
    // #8 — ancre LIVE : prolonge la courbe jusqu'a la valeur actuelle (sinon D/3D/1S restent figes)
    const lastMo = _fundLastMonthEUR(_MAP_OF(fond));
    if(A.length && valEUR>0 && lastMo>0){
      A = A.concat([{ d:today, v: A[A.length-1].v * (valEUR/lastMo) }]);
    }
    const win = A.filter(p=>p.d>=_hcut);
    // sous le mois (D/3D/1S) : montre [derniere cloture mensuelle → live] au lieu de figer sur 6 mois
  ...
  const _mkPerf = (name,color,valUSD,inv) => {
    ...
    if(pct==null){ const lastMo=_fundLastMonthEUR(_MAP_OF(fond)); pct = (lastMo>0) ? (valEUR/lastMo - 1) : (inv>0 ? valEUR/inv - 1 : 0); }   // court terme : live vs derniere cloture ; sinon depuis creation
  ...
  const _blendHeroPts = (()=>{
    ...
    const cMap={}, sMap={};
    _cmAnch.forEach(p=>{ if(p.d<=today) cMap[p.d]=p.v; });
    _smAnch.forEach(p=>{ if(p.d<=today) sMap[p.d]=p.v; });
    let dates=Object.keys(cMap).filter(d=>sMap[d]!=null).sort();
    // #8 — ancre LIVE blendee : prolonge jusqu'a aujourd'hui (sinon hero fige sous 7j)
    const lastC=_fundLastMonthEUR(_MAP_OF("CGIC")), lastS=_fundLastMonthEUR(_MAP_OF("CGIS"));
    if(dates.length && lastC>0 && lastS>0){
      const liveC=cMap[dates[dates.length-1]]*((cv*_ue)/lastC);
      const liveS=sMap[dates[dates.length-1]]*((sv*_ue)/lastS);
      cMap[today]=liveC; sMap[today]=liveS; if(!dates.includes(today)) dates.push(today);
    }
          ...
          <div style={{position:"relative"}}>
            ...
            {heroTFOpen && (
              <div style={{position:"absolute",top:"100%",left:0,marginTop:6,zIndex:30,background:C.bg2,border:`1px solid ${C.border2}`,borderRadius:C.radiusSm||10,padding:5,minWidth:150,boxShadow:"0 10px 28px rgba(0,0,0,0.45)"}}>
                {["D","3D","1S","1M","3M","YTD","1A","2A","ALL"].map(tf=>(
