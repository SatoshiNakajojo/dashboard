...
function computePortfolioModel(src){
  ...
  // ── Fonds JCGIC / JCGIS (money-weighted depuis INV) — formule UNIQUE, identique onglet JCGI ──
  let funds = null;
  try {
    const gp = (typeof calcGdbPrices==="function") ? calcGdbPrices(s) : {};
    const gcFonds = Math.round(gp.gdbCfondsUSD != null ? gp.gdbCfondsUSD : (s.crypto?s.crypto.total:0));
    const _si = (s.stocks&&s.stocks.items)||[];
    const gsFonds = Math.round(gp.gdbSfondsUSD || (_si.filter(x=>x.cat!=="Cash").reduce((a,x)=>a+(x.val||0),0) + ((_si.find(x=>x.t==="EURO")||{}).val||0)));
    const invG = (Array.isArray(inv)&&inv.length) ? inv : (typeof INV_SEED_OK!=="undefined"?INV_SEED_OK:[]);
    const netF = f => { let m=0,sh=0; invG.forEach(x=>{ if(x&&x.fonds===f){ const k=(x.io==="OUT")?-1:1; m+=k*(x.montant||0); sh+=k*(x.shares||0);} }); return {m,sh}; };
    const nc=netF("CGIC"), ns=netF("CGIS");
    funds = {
      cgic:{ valueUSD:gcFonds, valueEUR:Math.round(gcFonds*ue), invested:nc.m, parts:nc.sh,
        navUSD: nc.sh>0 ? gcFonds/nc.sh : 0, nav: nc.sh>0 ? (gcFonds/nc.sh)*ue : 0,
        pnlPct: nc.m>0 ? ((gcFonds*ue)/nc.m - 1)*100 : 0 },
      cgis:{ valueUSD:gsFonds, valueEUR:Math.round(gsFonds*ue), invested:ns.m, parts:ns.sh,
        navUSD: ns.sh>0 ? gsFonds/ns.sh : 0, nav: ns.sh>0 ? (gsFonds/ns.sh)*ue : 0,
        pnlPct: ns.m>0 ? ((gsFonds*ue)/ns.m - 1)*100 : 0 },
    };
  } catch(e){}
  return {
...
{chartData,hidden,EFF,eur,liveGSB,liveGDBS,liveBench,liveGC,liveDD,liveInv}){
  ...
  // #61 — P&L des fonds depuis le MODÈLE UNIQUE (même source que hero/snapshot/baromètre)
  const _jgFunds = (computePortfolioModel(src, _INV_GDB).funds) || {cgic:{},cgis:{}};
  const gcPerfAllTime = (_jgFunds.cgic.pnlPct||0)/100;   // P&L money-weighted en €
  const gsPerfAllTime = (_jgFunds.cgis.pnlPct||0)/100;

  // #61 — le snapshot (fonds inclus) est écrit par l'effet centralisé d'App (modèle unique sur EFF).
...
function App(){
  ...
  const handleRefresh = useCallback(async()=>{
    ...
    try{
      ...
      setLive(prev2=>{
        ...
        // #61 — le snapshot est désormais écrit par l'effet centralisé (sur EFF), plus ici.
        return prev2;
      });
  ...
  // #61 — Snapshot UNIQUE poussé au Worker depuis le modèle (EFF settled), quel que soit l'onglet ouvert
  const _snapRef = useRef(0);
  React.useEffect(()=>{
    try{
      const m = computePortfolioModel(EFF, liveInv||INV_SEED_OK);
      if(!m || !m.totalUSD) return;
      // anti-spam : n'écrit que si le total a bougé (ou >60s)
      const sig = m.totalUSD + Math.round((m.funds&&m.funds.cgis?m.funds.cgis.pnlPct:0)*100);
      if(sig===_snapRef.current) return;
      _snapRef.current = sig;
      const f = m.funds || {cgic:{},cgis:{}};
      saveBase('cgi_fund_stats',{
        totalUSD:m.totalUSD, totalEUR:m.totalEUR, cryptoUSD:m.cryptoUSD, stocksUSD:m.stocksUSD, cashUSD:m.cashUSD, usdEur:m.usdEur,
        cgic:{nav:(f.cgic&&f.cgic.nav)||0, valueEUR:(f.cgic&&f.cgic.valueEUR)||0, pnlPct:(f.cgic&&f.cgic.pnlPct!=null)?f.cgic.pnlPct:null},
        cgis:{nav:(f.cgis&&f.cgis.nav)||0, valueEUR:(f.cgis&&f.cgis.valueEUR)||0, pnlPct:(f.cgis&&f.cgis.pnlPct!=null)?f.cgis.pnlPct:null},
        ts:Date.now()
      });
    }catch(e){}
  }, [EFF, liveInv]);
