...
const Btn=({label,onClick,color,full,outline})=>(
  <button onClick={onClick} style={{background:outline?"transparent":(color||C.gold),border:`1px solid ${color||C.gold}`,borderRadius:10,padding:"12px 20px",color:outline?(color||C.gold):"#0B0A08",fontWeight:600,fontSize:13,cursor:"pointer",width:full?"100%":"auto",marginBottom:full?8:0}}>{label}</button>
);
...
function computePortfolioModel(src){
  ...
  sections.forEach(sec=>{
    let name;
    if(sec.n==="Crypto") name="Crypto";
    else if(sec.n==="Indices ETF") name="Indices";
    else if(sec.n==="Stock Picking") name="Picking";
...
function PageOverview({chartData,onSnapshot,eur,setEur,hidden,setHidden,EFF,refreshing,handleRefresh,refreshedAt,refreshErr,fromSnapshot,gistSync,liveDD,liveCM,liveGDBS,liveGC,chosenSource,iconDbVersion=0,bumpIconDb}){
  ...
  // #40 — courbe PROPRE à chaque fonds (Crypto/Actions) depuis ses rendements mensuels,
  // au lieu de réutiliser la courbe du total (qui rendait les deux mini-graphes incohérents).
  const _fundAnchors = (MAP) => {
    ...
    yrs.forEach(y=>{ const d=MAP[y]; if(!d||!d.m) return;
      d.m.forEach((ml,i)=>{ if(!ml) return;
        ...
        idx=idx*(1+ret);
        arr.push({d:y+"-"+String(i+1).padStart(2,"0")+"-28", v:idx});
      });
    });
    return arr;
  };
  const _cmAnch=_fundAnchors(typeof CRYPTO_MONTHLY!=="undefined"?CRYPTO_MONTHLY:{});
  const _smAnch=_fundAnchors(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:{});
  // #8 — valeur € de cloture du dernier mois connu (pour ancrer la courbe a la valeur live)
  const _fundLastMonthEUR = (MAP) => {
    let last=null; const today=new Date().toISOString().slice(0,10);
    const yrs=Object.keys(MAP||{}).filter(y=>/^\d{4}$/.test(y)).sort();
    yrs.forEach(y=>{ const d=MAP[y]; if(!d||!d.m) return;
      d.m.forEach((ml,i)=>{ if(ml==null) return;
        const ds=y+"-"+String(i+1).padStart(2,"0")+"-28"; if(ds>today) return;
        const base=(d.bom?d.bom[i]:0||0)+(d.inv?d.inv[i]:0||0);
        const v=base+(d.pnl?(d.pnl[i]||0):0); if(v>0) last=v;
      });
    });
    return last;
  };
  const _MAP_OF = fond => fond==="CGIC" ? (typeof CRYPTO_MONTHLY!=="undefined"?CRYPTO_MONTHLY:{}) : (typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:{});
  // #16 — journal quotidien : transforme chaque snapshot € en point d'indice et prolonge les ancres mensuelles
  const _daySnaps = (()=>{ const a=lsv9Get("cgi_daily"); return Array.isArray(a)?a:[]; })();
  const _enrichAnchors = (anch, MAP, field, liveEUR) => {
    const past = anch.filter(p=>p.d<=_po_today); // ignore les mois futurs (placeholders)
    if(!past.length) return anch;
    const lastMo=_fundLastMonthEUR(MAP);
    const lastV=past[past.length-1].v, lastD=past[past.length-1].d;
    if(!(lastMo>0)) return past;
    const extra=[];
    _daySnaps.forEach(s=>{ if(!s||!s.d||s.d<=lastD) return; const e=s[field]; if(!(e>0)) return; extra.push({d:s.d, v:lastV*(e/lastMo)}); });
    if(liveEUR>0){ const td=_po_today; const k=extra.findIndex(x=>x.d===td); const lv={d:td, v:lastV*(liveEUR/lastMo)}; if(k>=0) extra[k]=lv; else if(td>lastD) extra.push(lv); }
    extra.sort((a,b)=> a.d<b.d ? -1 : (a.d>b.d?1:0));
    return past.concat(extra);
  };
  const _cmAll=_enrichAnchors(_cmAnch, _MAP_OF("CGIC"), "c", _cryptoUSD*_ue);
  const _smAll=_enrichAnchors(_smAnch, _MAP_OF("CGIS"), "s", _stocksUSD*_ue);
  // ─── horizons réellement disponibles : un timeframe n'est proposé que s'il a >=2 vrais points ───
  const _cutOf = (tf) => {
    if(tf==="YTD") return new Date((new Date(Date.now()+NC_OFFSET_MS)).getUTCFullYear(),0,1).toISOString().slice(0,10);
    const d=_HTF_DAYS[tf]||30; return new Date(Date.now()+NC_OFFSET_MS-d*864e5).toISOString().slice(0,10);
  };
  const _blendDates = (()=>{ const cM={}, sM={}; _cmAll.forEach(p=>{if(p.d<=_po_today)cM[p.d]=1;}); _smAll.forEach(p=>{if(p.d<=_po_today)sM[p.d]=1;}); return Object.keys(cM).filter(d=>sM[d]).sort(); })();
  const _tfCount = (tf) => { const c=_cutOf(tf); return _blendDates.filter(d=>d>=c).length; };
  const _TF_LIST = (()=>{ const L=["D","3D","1S","1M","3M","YTD","1A","ALL"].filter(tf=>_tfCount(tf)>=2); return L.length?L:["ALL"]; })();
  React.useEffect(()=>{ if(_TF_LIST.indexOf(heroTF)<0) setHeroTF(_TF_LIST[_TF_LIST.length-1]); }, [_TF_LIST.join(","), heroTF]);
  const _fundSparkVals = (fond) => {
    const A = (fond==="CGIC"?_cmAll:_smAll).filter(p=>p.d<=_po_today); // mensuel + quotidien (journal #16)
    const win = A.filter(p=>p.d>=_hcut);
    const pts = win.length>=2 ? win : (A.length>=2 ? A.slice(-2) : A);
    return pts.length>=2 ? pts.map(p=>p.v) : [100,100];
  };
  const _mkPerf = (name,color,valUSD,inv) => {
    const valEUR = valUSD*_ue;
    const fond = name==="Crypto" ? "CGIC" : "CGIS";
    const sv = _fundSparkVals(fond);
    // % = rendement réel sur la fenêtre AFFICHÉE (premier→dernier point de la courbe) → toujours cohérent avec le tracé
    const pct = (sv.length>=2 && sv[0]>0) ? (sv[sv.length-1]/sv[0] - 1) : 0;
    const pnlEUR = Math.round(valEUR - valEUR/(1+pct));   // gain sur la période
    const pnl = eur ? pnlEUR : Math.round(pnlEUR/_ue);
    const up  = pct>=0;
    const _mn2=Math.min(...sv), _mx2=Math.max(...sv), _rng2=(_mx2-_mn2)||1, _n2=Math.max(1,sv.length-1);
    const pts = sv.map((v,i)=>`${(i/_n2*150).toFixed(1)},${(36-((v-_mn2)/_rng2)*28).toFixed(1)}`).join(" ");
  ...
  // courbe du patrimoine = indice mélangé (pondéré par valeur) ; le delta vient des MÊMES extrémités → cohérent
  const _blendWin = (()=>{
    const cv=_cryptoUSD||0, sv=_stocksUSD||0, tot=(cv+sv)||1;
    const cMap={}, sMap={};
    _cmAll.forEach(p=>{ if(p.d<=_po_today) cMap[p.d]=p.v; });
    _smAll.forEach(p=>{ if(p.d<=_po_today) sMap[p.d]=p.v; });
    const dates=Object.keys(cMap).filter(d=>sMap[d]!=null).sort();
    let win=dates.filter(d=>d>=_hcut);
    if(win.length<2) win=dates.slice(-2);
    return win.map(d=>(cv/tot)*cMap[d] + (sv/tot)*sMap[d]);
  })();
  const _blendPct = (_blendWin.length>=2 && _blendWin[0]>0) ? (_blendWin[_blendWin.length-1]/_blendWin[0] - 1) : 0;
  const _bUp = _blendPct>=0;
  const _blendCur = eur ? _sumEUR : _sumUSD;
  const _blendPnl = Math.round(_blendCur - _blendCur/(1+_blendPct));
  const _blendHeroPts = (()=>{
    const vals=_blendWin; if(vals.length<2) return "";
    const mn=Math.min(...vals), mx=Math.max(...vals), rng=(mx-mn)||1, n=Math.max(1,vals.length-1);
    return vals.map((v,i)=>`${(i/n*120).toFixed(1)},${(30-((v-mn)/rng)*26).toFixed(1)}`).join(" ");
  })();
...
function MarketStatus(){
  ...
  React.useEffect(()=>{ const id=setInterval(()=>setT(x=>x+1),60000); return ()=>clearInterval(id); },[]);
  const EXCH=[
  ...
  function info(ex){
    ...
    try{
      ...
      wd=g("weekday"); h=parseInt(g("hour"),10)%24; m=parseInt(g("minute"),10);
    }catch(e){ return {state:"closed",local:"—",label:"—"}; }
    const mins=h*60+m, local=String(h).padStart(2,"0")+":"+String(m).padStart(2,"0");
    if(wd==="Sat"||wd==="Sun") return {state:"closed",local,label:"Week-end"};
  ...
  return (
    <div style={{marginTop:24,marginBottom:8}}>
      ...
      <div style={{border:`1px solid ${C.border}`,borderRadius:C.radius||12,background:C.bg1,overflow:"hidden"}}>
        {rows.map((r,i)=>(
          <div key={r.n} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderTop:i?`1px solid ${C.border}`:"none"}}>
      ...
      <div style={{fontSize:9.5,color:C.text3,textAlign:"center",marginTop:8,opacity:.7}}>{nOpen} place{nOpen>1?"s":""} ouverte{nOpen>1?"s":""} · heure locale de chaque bourse</div>
    </div>
  );
}

function PageAllocation({hidden, EFF, eur=false, setEur, iconDbVersion=0, bumpIconDb, liveOk=false}){
  ...
  return(
    ...
    <div>
      ...
      {/* #47 — sélecteur Détail/Allocation, sous le bloc total */}
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        {[["detail","Live"],["ajust","Target"]].map(([v,l])=>(
          <button key={v} onClick={()=>setMode(v)} style={lxBtn({active:mode===v,style:{flex:1,padding:"9px 0",display:"flex",alignItems:"center",justifyContent:"center",gap:7}})}>
            {v==="detail"&&<span title={liveOk?"Cloud connecté · trades IBKR à jour":"Hors ligne ou trades non synchronisés"} style={{width:8,height:8,borderRadius:"50%",flexShrink:0,background:liveOk?"#22C55E":C.gray,boxShadow:liveOk?"0 0 6px #22C55E99":"none"}}/>}
            {l}
          </button>
        ))}
      </div>
...
function GdbCompareChartGDB({onTFChange, liveGSB, liveGDBS, liveBench, liveGC}){
  ...
  const _cgicA=_monthlyAnchors(typeof CRYPTO_MONTHLY!=="undefined"?CRYPTO_MONTHLY:{});
  const _cgisA=_monthlyAnchors(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:{});
  const gcRaw  = dates.map(d=>_interp(_cgicA,d));
  const gsRaw  = dates.map(d=>_interp(_cgisA,d));
  ...
  React.useEffect(()=>{
    try{
      ...
      saveBase('cgi_fund_stats',{..._cur,
        cgic:{nav:gcNAV*_ue, valueEUR:Math.round(gcFonds*_ue), pnlPct:gcPerfAllTime*100},
        cgis:{nav:gsNAV*_ue, valueEUR:Math.round(gsFonds*_ue), pnlPct:gsPerfAllTime*100},
        tsFunds:Date.now()});
...
{txns, liveFutures, hidden, eur, EFF, liveIbkrAnnex}){
  ...
  return (
    <div style={{padding:"8px 14px 96px"}}>
      <PageTitle title="History" sub={"Trades clôturés · "+(board==="spot"?"Spot":"Futures")}/>
    ...
    <div style={{background:C.bg1,border:`1px solid ${C.border}`,borderRadius:12,marginBottom:12,overflow:"hidden"}}>
      ...
      {open && (
        <div style={{padding:"0 16px 16px"}}>
          ...
          <button onClick={submit} style={{width:"100%",background:`linear-gradient(180deg, ${C.gold}, ${C.gold}DD)`,border:"none",borderRadius:9,padding:"11px",color:"#0B0A08",fontSize:12,fontWeight:700,letterSpacing:1,textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit"}}>Ajouter la transaction</button>
...
function cgiSetPinRaw(pin){ try{ if(pin) localStorage.setItem("cgi_pin",cgiPinHash(pin)); else localStorage.removeItem("cgi_pin"); }catch(e){} }
...
function PinLock({onUnlock}){
  ...
  React.useEffect(()=>{
    if(pin.length===4){
      if(cgiCheckPin(pin)){
        try{ sessionStorage.setItem("cgi_unlocked","1"); }catch(e){}
        onUnlock();
      } else {
        setErr(true);
  ...
  const press=(d)=>{ setErr(false); setPin(p=> p.length<4 ? p+d : p); };
...
function App(){
  ...
  const[unlocked,setUnlocked]=useState(()=>{ try{ return !cgiGetPin() || sessionStorage.getItem("cgi_unlocked")==="1"; }catch(e){ return true; } });
        ...
        if(res.ok){
          ...
          try{
            ...
            if(!_txnsMig){
              ...
              const merged = unionTxnsById(_localTx, _kvTx);
              if(merged.length !== _localTx.length || merged.length !== _kvTx.length){ setTxns(merged); lsv9Set('cgi_txns', merged); }
              if(merged.length > _kvTx.length){ saveBase('cgi_txns', merged); }
            }
  ...
  const applyPositionsFromTxns=useCallback(()=>{
    ...
    if(base.portfolio && base.portfolio.items && base.portfolio.items.length>0){
      ...
      try{ saveBase('cgi_crypto', tmpEFF.crypto); saveBase('cgi_stocks', tmpEFF.stocks); saveBase('cgi_bank', tmpEFF.bank); saveBase('cgi_portfolio', tmpEFF.portfolio); }catch(e){}
      return;
    }
    ...
    try{ saveBase('cgi_crypto', tmpEFF.crypto); saveBase('cgi_stocks', tmpEFF.stocks); saveBase('cgi_bank', tmpEFF.bank); }catch(e){}
  },[live,txns]);
  ...
  React.useEffect(()=>{
    if(txns && txns.length>0 && live && !live._fromTxnsApplied){
  ...
  return(
    <div key={themeName} style={{fontFamily:C.font||"'-apple-system',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,maxWidth:430,margin:"0 auto",paddingBottom:78,boxShadow:themeName==="midnight"?"0 0 80px rgba(180,100,240,.08)":themeName==="bitcoin"?"0 0 80px rgba(247,147,26,.06)":"none"}}>
      <div style={{
        ...
        <div onClick={()=>setTab(0)} title="Accueil" style={{display:"flex",alignItems:"center",gap:1,cursor:"pointer",minWidth:0}}>
      ...
      <div style={{padding:"0 16px"}}>
        ...
        {tab===1 && <PageAllocation hidden={hidden} EFF={EFF} eur={eur} setEur={setEur} iconDbVersion={iconDbVersion} bumpIconDb={bumpIconDb} liveOk={liveOk}/>}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:C.bg1,borderRadius:"20px 20px 0 0",padding:"20px 20px 36px",width:"100%",maxWidth:430,maxHeight:"85vh",overflowY:"auto",border:`1px solid ${C.border}`}}>
            ...
            <button onClick={()=>{
              if(cgiGetPin()){
                ...
                if(np===""){ cgiSetPinRaw(""); alert("🔓 Verrouillage désactivé."); return; }
                if(!/^\d{4}$/.test(np)){ alert("Il faut exactement 4 chiffres."); return; }
                cgiSetPinRaw(np); alert("✅ Code mis à jour.");
              } else {
