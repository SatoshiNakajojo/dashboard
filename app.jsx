...
// #90 — Payload cgi_bank « sûr » pour les écritures NON-ÉDITRICES (snapshot quotidien, persistance
// post-trade). Renvoie le cgi_bank LOCAL (= valeur cloud adoptée après réconciliation) avec son
// savedAt PRÉSERVÉ, jamais un Date.now() frais et jamais le breakdown du seed (EFF/CURRENT.bank).
// Sans ça, le snapshot ré-écrivait chaque jour l'ancien solde avec un horodatage récent qui gagnait
// le garde-fou du worker (exTs>inTs) et écrasait l'édition Cash Matelas faite sur l'iPhone.
// Seuls l'observateur de solde et le dépôt/retrait tamponnent légitimement Date.now().
function bankSnapshotSafe(fallbackBank){
  try{ var b=lsv9Get('cgi_bank'); if(b && b.breakdown) return {...b}; }catch(e){}
  var out={...((fallbackBank)||{})}; delete out.savedAt; // pas de savedAt → traité comme ts=0, ne peut jamais gagner
  return out;
}

// #90 — Réglage MANUEL et AUTO-VÉRIFIÉ du Cash Matelas. LIT d'abord le savedAt du cloud, écrit avec un
// horodatage GARANTI supérieur (impossible de perdre le duel du garde-fou worker), puis RELIT (anti-cache)
// pour confirmer. Rafraîchit l'affichage sans rechargement. Outil de secours + test réseau sans ambiguïté :
//   __cgiSetMatelas(7159, 838, 0)            // LCL, BCI, DeBlock (en €)
//   __cgiSetMatelas({LCL:7200, BCI:900})     // objet partiel accepté (les comptes omis gardent leur valeur)
async function __cgiSetMatelas(lcl, bci, deblock){
  try{
    // 1. construire le nouveau breakdown (à partir de l'existant pour ne rien perdre)
    var cur={}; try{ var b0=lsv9Get('cgi_bank'); cur=(b0&&b0.breakdown)||{}; }catch(e){}
    var bd={LCL:cur.LCL||0, BCI:cur.BCI||0, DeBlock:cur.DeBlock||0};
    if(lcl && typeof lcl==="object"){ Object.keys(lcl).forEach(function(k){ bd[k]=Number(lcl[k])||0; }); }
    else {
      if(lcl!=null && lcl!=="") bd.LCL=Number(lcl)||0;
      if(bci!=null && bci!=="") bd.BCI=Number(bci)||0;
      if(deblock!=null && deblock!=="") bd.DeBlock=Number(deblock)||0;
    }
    var total=Object.keys(bd).reduce(function(s,k){ return s+(bd[k]||0); },0);

    // 2. LIRE le savedAt actuel du cloud, puis forcer un savedAt garanti gagnant (le garde-fou worker ne
    //    remplace que si entrant > existant ; un solde bloqué peut porter un savedAt >= horloge locale).
    var exTs=0;
    try{
      var pr=await fetch(CF_WORKER_URL+"/read?k="+encodeURIComponent(CF_AUTH_KEY)+"&_t="+Date.now(), { headers:{"X-Auth-Key":CF_AUTH_KEY}, cache:"no-store", signal:AbortSignal.timeout(25000) });
      if(pr.ok){ var pkv=await pr.json(); exTs=(pkv.cgi_bank&&pkv.cgi_bank.savedAt)||0; }
    }catch(e){}
    var now=Date.now();
    var savedAt=Math.max(now, (exTs||0)+5000); // +5s de marge → bat le garde-fou et l'observateur concurrent
    if(exTs && exTs > now+600000) console.warn("[matelas] ⚠️ le savedAt du cloud ("+new Date(exTs).toISOString()+") est TRÈS en avance sur l'horloge locale ("+new Date(now).toISOString()+") — c'est ce qui bloquait les écritures. On force au-dessus.");
    console.info("[matelas] duel savedAt → cloud existant="+exTs+" · écriture="+savedAt+(savedAt>exTs?" (gagne ✔)":" (PROBLÈME)"));
    var d=(function(){ try{ return today(); }catch(e){ return new Date().toISOString().slice(0,10); } })();
    var newBank={date:d, breakdown:{...bd}, totalEUR:total, savedAt:savedAt};

    // 3. écrire en LOCAL tout de suite + réaligner les lignes "Cash Matelas" du portfolio
    try{ lsv9Set('cgi_bank', newBank); }catch(e){}
    var patchItems=function(key){ try{ var g=lsv9Get(key); if(!g||!g.items) return null;
      g.items=g.items.map(function(it){ if(it.cat!=="Cash Matelas") return it;
        var kk=Object.keys(bd).find(function(k){ return k.toUpperCase()===(it.t||"").toUpperCase(); });
        if(kk==null) return it; var v=bd[kk]||0;
        return {...it, valEUR:v, qty:v, val:Math.round(v*1.086957)}; });
      lsv9Set(key,g); return g; }catch(e){ return null; } };
    var pPort=patchItems('cgi_portfolio'); patchItems('cgi_stocks');

    // 4. rafraichir l affichage sans reload si le hook est branché
    try{ if(typeof window!=="undefined" && typeof window.__cgiApplyBankLive==="function") window.__cgiApplyBankLive(bd, total); }catch(e){}

    // 5. POUSSER au cloud (batch, timeout long, retries) — cgi_bank + les lignes Cash Matelas du portfolio
    var body={ cgi_bank:newBank };
    if(pPort) body.cgi_portfolio=pPort;
    var pushed=false, lastErr=null;
    for(var att=1; att<=4 && !pushed; att++){
      try{
        var res=await fetch(CF_WORKER_URL+"/write-bases",{ method:"POST",
          headers:{"Content-Type":"application/json","X-Auth-Key":CF_AUTH_KEY},
          body:JSON.stringify(body), signal:AbortSignal.timeout(25000) });
        if(res.ok){ pushed=true; break; } lastErr="HTTP "+res.status;
      }catch(e){ lastErr=(e&&e.message)||String(e); }
      if(!pushed) await new Promise(function(r){ setTimeout(r, 600*att); });
    }
    if(!pushed){ try{ lsv9MarkDirty&&lsv9MarkDirty('cgi_bank'); }catch(e){}
      console.error("[matelas] ❌ ÉCHEC d'écriture au cloud ("+lastErr+"). Gardé en LOCAL mais N'A PAS atteint le cloud → réseau (VPN / pare-feu / workers.dev bloqué ?). Réessaie sur une autre connexion.");
      return {ok:false, reason:"push", detail:lastErr, breakdown:bd}; }

    // 6. RELIRE le cloud (anti-cache) pour CONFIRMER que la valeur y est bien
    await new Promise(function(r){ setTimeout(r, 400); }); // laisse KV propager
    var confirmed=false, cloudBd=null, cloudTs=null;
    try{
      var rr=await fetch(CF_WORKER_URL+"/read?k="+encodeURIComponent(CF_AUTH_KEY)+"&_t="+Date.now(), { headers:{"X-Auth-Key":CF_AUTH_KEY}, cache:"no-store", signal:AbortSignal.timeout(25000) });
      if(rr.ok){ var kv=await rr.json(); cloudBd=(kv.cgi_bank&&kv.cgi_bank.breakdown)||null; cloudTs=(kv.cgi_bank&&kv.cgi_bank.savedAt)||null;
        if(cloudBd){ confirmed=Object.keys(bd).every(function(k){ return Math.round(cloudBd[k]||0)===Math.round(bd[k]||0); }); } }
    }catch(e){ lastErr=(e&&e.message)||String(e); }
    if(confirmed){
      console.info("[matelas] ✅ CONFIRMÉ dans le cloud : "+JSON.stringify(bd)+" (total "+total+" €, savedAt="+cloudTs+"). Recharge l'AUTRE appareil pour qu'il l'adopte.");
      return {ok:true, confirmed:true, breakdown:bd, totalEUR:total, savedAt:cloudTs};
    }
    console.error("[matelas] ❌ le cloud n'a PAS pris la nouvelle valeur (relecture : "+JSON.stringify(cloudBd)+" savedAt="+cloudTs+", erreur : "+lastErr+"). Si savedAt cloud ≥ écriture, c'est le garde-fou ; relance l'appel.");
    return {ok:false, reason:"guard", breakdown:bd, cloudBreakdown:cloudBd, cloudSavedAt:cloudTs};
  }catch(e){ console.error("[matelas] erreur inattendue :", e&&e.message); return {ok:false, reason:"exception", detail:e&&e.message}; }
}
try{ if(typeof window!=="undefined") window.__cgiSetMatelas=__cgiSetMatelas; }catch(e){}
...
function BankModal({ bank, label, valueEUR=0, eur=false, usdEur=0.92, hidden=false, onClose }){
  ...
  // #90 — éditeur tactile du solde (utilise __cgiSetMatelas : écriture forcée + relecture confirmée)
  const [edit,setEdit]=useState(false);
  const [amt,setAmt]=useState(String(Math.round(valueEUR)));
  const [busy,setBusy]=useState(false);
  const [msg,setMsg]=useState(null);
  React.useEffect(()=>{ setAmt(String(Math.round(valueEUR))); },[valueEUR]);
  const canonKey=(()=>{ try{ const b=lsv9Get('cgi_bank'); const ks=Object.keys((b&&b.breakdown)||{}); const m=ks.find(k=>k.toUpperCase()===String(bank||"").toUpperCase()); return m||bank; }catch(e){ return bank; } })();
  const doSaveBank=async()=>{
    if(busy) return;
    const v=Math.round(Number(String(amt).replace(/[^0-9.\-]/g,''))||0);
    setBusy(true); setMsg({t:"wait",x:"Écriture + vérification cloud…"});
    try{
      const fn=(typeof window!=="undefined")&&window.__cgiSetMatelas;
      const r=fn ? await fn({[canonKey]:v}) : {ok:false,reason:"indispo"};
      if(r&&r.ok&&r.confirmed) setMsg({t:"ok",x:"✅ Enregistré et confirmé dans le cloud. Recharge l'autre appareil pour qu'il l'adopte."});
      else if(r&&r.reason==="push") setMsg({t:"err",x:"❌ Réseau : l'écriture n'est pas parvenue au cloud. Vérifie ta connexion et réessaie."});
      else if(r&&r.reason==="guard") setMsg({t:"err",x:"❌ Le cloud a refusé la valeur (garde-fou). Redéploie le worker v70-cgi, puis réessaie."});
      else if(r&&r.reason==="indispo") setMsg({t:"err",x:"❌ Outil indisponible (recharge l'app en v7.34+)."});
      else setMsg({t:"err",x:"❌ Échec ("+((r&&r.reason)||"inconnu")+")."});
    }catch(e){ setMsg({t:"err",x:"❌ Erreur : "+(e&&e.message)}); }
    setBusy(false);
  };
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:80,background:"rgba(0,0,0,.72)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:560,maxHeight:"82vh",overflowY:"auto",background:C.bg2,border:`1px solid ${C.border2}`,borderRadius:"16px 16px 0 0",padding:"16px 16px 28px"}}>
        ...
        {/* #90 — éditeur tactile du solde (en haut, impossible à rater) */}
        <div style={{marginBottom:14,paddingBottom:12,borderBottom:`1px solid ${C.border2}`}}>
          {!edit ? (
            <button onClick={()=>{ setMsg(null); setEdit(true); }} style={{width:"100%",padding:"12px",background:C.green,border:"none",borderRadius:10,color:"#04120A",fontSize:14,fontWeight:700,cursor:"pointer"}}>✎ Modifier le solde</button>
          ) : (
            <div>
              <div style={{fontSize:11,color:C.text3,marginBottom:6}}>Nouveau solde {label||bank} (en €)</div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input type="number" inputMode="numeric" value={amt} onChange={e=>setAmt(e.target.value)} disabled={busy} autoFocus
                  style={{flex:1,minWidth:0,padding:"11px 12px",background:C.bg,border:`1px solid ${C.border2}`,borderRadius:10,color:C.text,fontSize:16,fontVariantNumeric:"tabular-nums",outline:"none"}} />
                <button onClick={doSaveBank} disabled={busy}
                  style={{padding:"11px 16px",background:busy?C.bg:C.green,border:"none",borderRadius:10,color:busy?C.text3:"#04120A",fontSize:14,fontWeight:700,cursor:busy?"default":"pointer",whiteSpace:"nowrap"}}>{busy?"…":"Valider"}</button>
                <button onClick={()=>{ setEdit(false); setMsg(null); setAmt(String(Math.round(valueEUR))); }} disabled={busy}
                  style={{padding:"11px 12px",background:"none",border:`1px solid ${C.border2}`,borderRadius:10,color:C.text3,fontSize:13,cursor:"pointer"}}>Annuler</button>
              </div>
            </div>
          )}
          {msg && <div style={{marginTop:8,fontSize:12,lineHeight:1.4,color:msg.t==="ok"?C.green:(msg.t==="err"?C.red:C.text3)}}>{msg.x}</div>}
        </div>
        {n>1 ? (
...
function TradeModal({onClose, onAdd, onTradeApplied, EFF, holders, onInvestApplied}){
  ...
  const submitDepot=()=>{
    ...
    // #90 — router le solde par la mécanique robuste : met à jour cgi_bank ET les lignes portfolio
    // "Cash Matelas" (source de l'affichage/snapshot) ET le cloud (écriture forcée + relecture confirmée).
    // Sans ça, un dépôt ne changeait que cgi_bank : l'affichage/snapshot restaient sur l'ancien montant.
    try{
      if(typeof window!=="undefined" && typeof window.__cgiSetMatelas==="function"){ window.__cgiSetMatelas({...newBank.breakdown}); }
      else if(typeof saveBase==="function"){ saveBase('cgi_bank', {...newBank, savedAt:Date.now()}); }
    }catch(e){ try{ if(typeof saveBase==="function") saveBase('cgi_bank', {...newBank, savedAt:Date.now()}); }catch(e2){} }
...
function computeClosedTrades(txns){
  ...
  var closed=[];
  var open=[]; // #2b — positions encore ouvertes (pos > 0 en fin de journal) : à afficher dans History
  Object.keys(byT).forEach(function(tk){
    ...
    // #2b — reste une position ouverte à la fin ? (ex. PLTR : 77 accumulées, 27 restantes)
    // On la remonte comme un trade « ouvert » : mêmes entrées/sorties, P&L RÉALISÉ sur la part déjà
    // vendue, parts restantes affichées. exitDate=null (badge « ouvert »), lastDate = dernière opération.
    if(cyc && pos>EPS){
      var _avg=cyc.buyQty?cyc.buyVal/cyc.buyQty:0;        // PRU (coût moyen pondéré)
      var _soldCost=_avg*cyc.sellQty;                     // coût des parts vendues
      var _realized=cyc.sellVal-_soldCost;                // P&L réalisé sur les sorties partielles
      var _lastDate=cyc.fills.length?cyc.fills[cyc.fills.length-1].date:cyc.entryDate;
      var _dur=Math.round((new Date(_lastDate)-new Date(cyc.entryDate))/864e5);
      open.push({ticker:tk, src:cyc.src, entryDate:cyc.entryDate, exitDate:null, lastDate:_lastDate,
        durationDays:_dur, qty:cyc.buyQty, remainingQty:Math.round(pos*1e6)/1e6,
        entryPrice:_avg, exitPrice:(cyc.sellQty?cyc.sellVal/cyc.sellQty:null),
        investedUSD:cyc.buyVal, remainingCostUSD:Math.max(0,_avg*pos),
        pnlUSD:_realized, pct:(_soldCost>EPS?_realized/_soldCost*100:null),
        nBuy:cyc.nBuy, nSell:cyc.nSell, fills:cyc.fills, open:true});
    }
  });
...
function TradeDetailModal({trade, kind, onClose, liveIbkrAnnex}){
  ...
  const isOpen = !isFut && !!trade.open; // #2b — position encore ouverte
  ...
  // #2b — pour une position ouverte, pas de date de sortie : on borne le graphe et l'annexe à aujourd'hui.
  const _todayStr = (function(){ try{ return new Date().toISOString().slice(0,10); }catch(e){ return exitDate; } })();
  const effExit = exitDate || (isOpen ? _todayStr : exitDate);
  const pnlUSD = trade.pnlUSD;
  const up = pnlUSD>=0;
  const eurRate = usdEurAt(effExit);
  ...
  if(src==="ibkr"){
    (liveIbkrAnnex||[]).forEach(function(a){
      if(a && a.ticker===ticker && a.date>=entryDate && a.date<=effExit){
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:680,background:"rgba(0,0,0,.78)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:C.bg1,borderRadius:"20px 20px 0 0",padding:"20px 16px 30px",width:"100%",maxWidth:460,maxHeight:"90vh",overflowY:"auto",border:`1px solid ${C.border}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7,marginBottom:14}}>
          <Info k={isOpen?"Prix de revient (PRU)":"Entree"} v={isOpen ? ((trade.entryPrice||0).toFixed((trade.entryPrice||0)<10?4:2)+" · depuis "+entryDate) : (entryDate+" \u00b7 "+(trade.entryPrice||0).toFixed( (trade.entryPrice||0)<10?4:2 ))}/>
          {isOpen
            ? <Info k="Statut" v={"En cours \u00b7 "+(trade.remainingQty||0).toLocaleString("fr-FR",{maximumFractionDigits:6})+" parts"} c={C.gold}/>
            : <Info k="Sortie" v={exitDate+" \u00b7 "+(trade.exitPrice||0).toFixed( (trade.exitPrice||0)<10?4:2 )}/>}
          <Info k="Duree" v={trade.durationDays+" j"+(isOpen?" (en cours)":"")}/>
          {isFut
            ? <Info k="Notionnel / Marge" v={fU(trade.notionalUSD)+" / "+fU(trade.marginUSD)}/>
            : <Info k={isOpen?"Quantite (achetee / restante)":"Quantite"} v={isOpen ? ((trade.qty||0).toLocaleString("fr-FR",{maximumFractionDigits:6})+" / "+(trade.remainingQty||0).toLocaleString("fr-FR",{maximumFractionDigits:6})) : (trade.qty||0).toLocaleString("fr-FR",{maximumFractionDigits:6})}/>}
          {isFut
            ? <Info k="Funding / Frais" v={Math.round(trade.raw.fundingUSD)+" / "+Math.round(trade.raw.tradingFeesUSD)+" $"}/>
            : <Info k={isOpen?"Capital encore investi":"Capital investi"} v={fU(isOpen?(trade.remainingCostUSD||0):trade.investedUSD)}/>}
...
function PageWatchlist({ EFF, hidden }){
  ...
  useEffect(function(){
    ...
    fetch(CF_WORKER_URL+"/read?k="+encodeURIComponent(CF_AUTH_KEY)+"&_t="+Date.now(),{headers:{"X-Auth-Key":CF_AUTH_KEY},cache:"no-store",signal:AbortSignal.timeout(18000)})
      .then(function(r){return r.json();})
...
{txns, liveFutures, hidden, eur, EFF, liveIbkrAnnex, ibkrTrades=[]}){
  ...
  // #2b — + positions ENCORE OUVERTES (badge « ouvert », parts restantes) sur la même source fusionnée.
  const spot = React.useMemo(function(){ var r=computeClosedTrades(mergedTx); return (r.closed||[]).concat(r.open||[]); }, [mergedTx]);
  ...
  const sorted = list.slice().sort(function(a,b){
    if(sortK==="date"){ var da=new Date(a.exitDate||a.lastDate||a.entryDate||0), db=new Date(b.exitDate||b.lastDate||b.entryDate||0); return db-da; }
    if(sortK==="pnl") return b.pnlUSD-a.pnlUSD;
    if(sortK==="pct") return (b.pct==null?-1e12:b.pct)-(a.pct==null?-1e12:a.pct);
    return b.durationDays-a.durationDays;
  });
  // #2b — stats globales calculées sur les trades CLÔTURÉS uniquement (win rate, durée moyenne, etc.
  // n'ont pas de sens sur une position encore ouverte). Les positions ouvertes s'affichent dans la
  // liste mais ne faussent pas ces indicateurs.
  const statList = list.filter(function(t){ return !t.open; });
  ...
  return (
    <div style={{padding:"8px 14px 96px"}}>
      <PageTitle title="History" sub={board==="spot"?"Trades clôturés + positions ouvertes · Spot (saisies + IBKR unifiés)":"Trades clôturés · Futures"}/>
      ...
      <div>
        ...
        {sorted.length===0 && <div style={{textAlign:"center",color:C.text3,fontSize:13,padding:30}}>{board==="spot"?"Aucun trade ni position.":"Aucun trade clôturé."}</div>}
      </div>
...
{EFF, hidden, txns, addTxn, delTxn, applyPositions, chartData, liveDD, liveGDBS, liveGC, liveGSB, liveCM, liveSM, liveTM, liveBench, liveInv, liveFutures, liveIbkrAnnex, kvRefreshTick, handleRefresh, refreshing, gistSync, onOpenKvDiag}){
  ...
  function doLoadCloud(){
    ...
    setCloudError(null);
    fetch(CF_WORKER_URL+"/read?k="+encodeURIComponent(CF_AUTH_KEY)+"&_t="+Date.now(), {
      headers:{"X-Auth-Key":CF_AUTH_KEY},
      cache:"no-store",
      signal: AbortSignal.timeout(18000),
    })
...
function App(){
  ...
  useEffect(()=>{
    ...
    // #90 — OBSERVATEUR NEUTRALISÉ. Le Cash Matelas suit désormais le même chemin que crypto/stocks :
    // édition → __cgiSetMatelas (cloud+local) ; boot → adoption (cloud→local) ; rebuild → reconstruit
    // les lignes depuis cgi_bank local. L'observateur ne doit donc PLUS rien écrire : il réécrivait
    // l'ancienne valeur en local entre l'adoption et le rebuild (visible dans la console). No-op.
    _lastBankSig.current=sig;
  }, [live]);
  useEffect(()=>{
    if(!cloudBankSync || !live) return;
    const _bd=cloudBankSync.breakdown||{};
    // #86j — dédup et déclenchement par SIGNATURE DE CONTENU (pas par horodatage). L'observateur
    // du PC re-tamponnait le solde local à chaque refresh, le rendant faussement « plus récent »
    // que le cloud → l'ancien garde temporel annulait la réconciliation. Comme tout édit local est
    // publié au cloud immédiatement, le cloud fait autorité : on aligne dès que le contenu diffère.
    const cloudSig=Object.keys(_bd).sort().map(k=>k+":"+Math.round(_bd[k]||0)).join("|");
    const curBd=(live.bank&&live.bank.breakdown)||{};
    const liveSig=Object.keys(curBd).sort().map(k=>k+":"+Math.round(curBd[k]||0)).join("|");
    if(cloudSig===liveSig){ _bankSyncDone.current=cloudSig; return; } // déjà aligné
    if(_bankSyncDone.current===cloudSig) return; // déjà appliqué cette signature
    _bankSyncDone.current=cloudSig;
    const cbTs=cloudBankSync.savedAt||Date.now();
    ...
    // marquer la signature comme "déjà poussée" pour que l'observateur ne re-pousse pas ce contenu
    try{ _lastBankSig.current=cloudSig; }catch(e){}
    const _next={...live,
    ...
    setLive(_next);
    console.info("[bank] Cash Matelas réconcilié sur le cloud ("+cloudSig+")");
  }, [cloudBankSync, live]);
  // #90 — hook pour __cgiSetMatelas : applique un breakdown à l'affichage sans rechargement,
  // en réutilisant la voie de réconciliation existante (setCloudBankSync → effet ci-dessus).
  useEffect(()=>{
    try{ window.__cgiApplyBankLive=function(bd,total){
      try{ var t=(total!=null)?total:Object.values(bd||{}).reduce(function(s,v){return s+(v||0);},0);
        setCloudBankSync({breakdown:{...bd}, savedAt:Date.now()+1, totalEUR:t}); }catch(e){} }; }catch(e){}
    return function(){ try{ if(window.__cgiApplyBankLive) delete window.__cgiApplyBankLive; }catch(e){} };
  }, []);
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try {
        const res=await fetch(CF_WORKER_URL+"/read?k="+encodeURIComponent(CF_AUTH_KEY)+"&_t="+Date.now(),{headers:{"X-Auth-Key":CF_AUTH_KEY},cache:"no-store",signal:AbortSignal.timeout(18000)});
        if(res.ok){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try {
        const res = await fetch(CF_WORKER_URL+"/read?k="+encodeURIComponent(CF_AUTH_KEY)+"&_t="+Date.now(),{
          headers:{"X-Auth-Key":CF_AUTH_KEY},
          cache:"no-store",
          signal: AbortSignal.timeout(18000),
        });
  ...
  const doSnapUpload = useCallback(async()=>{
    ...
    try {
      ...
      try{ lsv9Set('cgi_bank',  bankSnapshotSafe(_srcPos.bank || CURRENT.bank)); }catch(e){} // #90 LOCAL SEUL — le cloud cgi_bank n'est écrit que par les éditions explicites
    ...
    for(let attempt = 1; attempt <= 3 && !basesOk; attempt++){
      try {
        const bases = {
          ...
          // #90 — cgi_bank VOLONTAIREMENT EXCLU du snapshot cloud : il n'est écrit dans le cloud QUE
          // par les éditions explicites (__cgiSetMatelas). Ainsi un boot/snapshot ne peut jamais
          // écraser le solde édité sur un autre appareil.
  ...
  const applyPositionsFromTxns=useCallback(()=>{
    ...
    if(base.portfolio && base.portfolio.items && base.portfolio.items.length>0){
      ...
      // #90 — CASH MATELAS RECONSTRUIT depuis la source autoritaire cgi_bank (local, gardé synchro
      // avec le cloud par l'adoption), EXACTEMENT comme crypto/stocks sont reconstruits depuis les txns.
      // Avant, le rebuild PRÉSERVAIT d'anciennes lignes Cash Matelas → il écrasait la valeur fraîche
      // que la réconciliation venait d'appliquer (l'affichage divergeait du cloud). Fini.
      var _bankBd=(function(){ try{ var b=lsv9Get('cgi_bank'); return (b&&b.breakdown)||null; }catch(e){ return null; } })();
      if(_bankBd){
        var _seenMat={};
        newItems=newItems.map(function(it){ if(it.cat!=="Cash Matelas") return it;
          var kk=Object.keys(_bankBd).find(function(k){ return k.toUpperCase()===(it.t||"").toUpperCase(); });
          if(kk==null) return it; _seenMat[kk]=1; var v=_bankBd[kk]||0;
          return {...it, qty:v, valEUR:v, pa:1, live:eurUsd, val:Math.round(v*eurUsd), pnl:0, pct:0}; });
        Object.keys(_bankBd).forEach(function(k){ if(_seenMat[k]) return; var v=_bankBd[k]||0;
          newItems=newItems.concat([{t:k, cat:"Cash Matelas", qty:v, valEUR:v, pa:1, live:eurUsd, val:Math.round(v*eurUsd), pnl:0, pct:0, _fromBank:true}]); });
      }
      ...
      const tmpEFF={...base, portfolio:{...base.portfolio, items:newItems}, crypto:{...base.crypto, total:cryptoTotal, items:cryptoItems}, stocks:{...base.stocks, total:stocksTotal, items:stocksItems}, bank:{...base.bank, breakdown:(_bankBd?{..._bankBd}:base.bank.breakdown), totalEUR:bankTotalEUR}};
      ...
      try{ saveBase('cgi_crypto', tmpEFF.crypto); saveBase('cgi_stocks', tmpEFF.stocks);
        ...
        try{ lsv9Set('cgi_bank', {...tmpEFF.bank, ...(_bkSA?{savedAt:_bkSA}:{})}); }catch(e){} /* #90 local seul */
    ...
    try{ saveBase('cgi_crypto', tmpEFF.crypto); saveBase('cgi_stocks', tmpEFF.stocks);
      ...
      try{ lsv9Set('cgi_bank', {...tmpEFF.bank, ...(_bkSA2?{savedAt:_bkSA2}:{})}); }catch(e){} /* #90 local seul */ }catch(e){}
  },[live,txns]);
  ...
  useEffect(()=>{
    ...
    try {
      ...
      if(live.bank){ try{ lsv9Set('cgi_bank', bankSnapshotSafe(live.bank)); }catch(e){} } /* #90 local seul (post-trade) */
        ...
        <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:C.bg1,borderRadius:"20px 20px 0 0",padding:"20px 20px 36px",width:"100%",maxWidth:430,maxHeight:"85vh",overflowY:"auto",border:`1px solid ${C.border}`}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {[
                ["v7.41","History affiche aussi les positions OUVERTES (#2b). Jusqu'ici History ne montrait que les trades clôturés (aller-retour complet) : une position encore en cours (ex. PLTR — 77 parts accumulées, 27 restantes après des sorties partielles) n'apparaissait pas. Désormais chaque position ouverte figure sur une ligne unique, badge « OUVERT », avec ses parts restantes, et le clic ouvre le même drill-down (graphe Yahoo borné à aujourd'hui + tous les points d'entrée et de sortie). Le P&L montré pour une position ouverte est le P&L RÉALISÉ sur la part déjà vendue (le PRU et le capital encore investi sont détaillés dans le drill-down). Les indicateurs globaux (P&L total, win rate, meilleur/pire, durée moyenne) restent calculés sur les trades clôturés uniquement, pour ne pas être faussés par les positions en cours. Tri par dernière opération pour les positions ouvertes."],
                ["v7.40","Cash matelas — MÊME SYSTÈME QUE CRYPTO/STOCKS (#90). La console a montré la mécanique exacte : la réconciliation appliquait la bonne valeur du cloud (LCL:6400|BCI:1000) PUIS le rebuild la remettait à l'ancienne (LCL:7159|BCI:838), car il PRÉSERVAIT les anciennes lignes Cash Matelas au lieu de les reconstruire — la seule différence avec crypto/stocks, qui eux sont reconstruits depuis cgi_txns (qui se synchronise parfaitement). Correctif de fond : le Cash Matelas suit désormais le même chemin — le rebuild RECONSTRUIT les lignes depuis la source autoritaire cgi_bank local (gardée synchro avec le cloud par l'adoption), au lieu de préserver d'anciennes valeurs. L'observateur, qui réécrivait l'ancienne valeur en local entre l'adoption et le rebuild, est neutralisé. Combiné aux versions précédentes (cache /read corrigé, cloud cgi_bank écrit uniquement par les éditions), la chaîne est enfin cohérente : édition → __cgiSetMatelas (cloud+local) → adoption au boot → rebuild depuis cgi_bank → affichage. Worker v71-cgi et sw inchangés."],
                ["v7.39","Cash matelas — UN SEUL écrivain cloud autorisé (#90). La console a montré la cause finale : le PC écrivait cgi_bank → KV plusieurs fois AU BOOT, avant même de lire le cloud, avec l'ancien solde et un horodatage frais → il écrasait dans le cloud l'édition faite sur l'iPhone (cloud=local=maintenant, mais ancienne valeur). Trop d'écrivains concurrents (rebuild, snapshot, observateur, boot) rendaient tout garde-fou d'horodatage inefficace. Correctif de fond : le cloud cgi_bank n'est désormais écrit QUE par les éditions explicites (__cgiSetMatelas ← éditeur tactile & Dépôt/Retrait). Tous les autres chemins (rebuild, snapshot, observateur, post-trade) écrivent LOCAL SEUL — ils ne peuvent plus rien écraser. Le cloud cgi_bank = « la dernière valeur qu'un humain a tapée ». Le PC lit ce solde au boot et l'adopte pour l'affichage, sans jamais le réécrire. Worker v71-cgi (horodatage) inchangé. Après déploiement, refaire UNE édition sur l'iPhone pour repartir de la bonne valeur."],
                ["v7.38","Cash matelas — retour au garde-fou par HORODATAGE, le bon (#90). Le garde-fou « par contenu » (v70) était une erreur : il laissait n'importe quelle écriture au contenu différent gagner, donc les re-publications du PC au boot (observateur avec l'ancien solde) ÉCRASAIENT dans le cloud l'édition fraîche faite sur l'iPhone (console : « cgi_bank cloud ignoré » + saveBase cgi_bank répétés). Correctifs : (1) worker v71-cgi — retour au garde-fou par savedAt (le plus récent gagne) ; c'est sûr car les éditions passent par __cgiSetMatelas qui lit le savedAt du cloud et écrit AU-DESSUS (gagnent toujours), et les écritures non-éditrices préservent le savedAt (ne peuvent jamais écraser). (2) L'observateur de solde ne fabrique plus de Date.now() : il préserve le savedAt existant. L'affichage suit cgi_bank via la réconciliation post-boot (aligne aussi les lignes portfolio). À déployer : worker v71-cgi + app v7.38, puis refaire UNE édition (elle forcera un savedAt gagnant et se propagera)."],
                ["v7.37","Cash matelas — LE coupable de fond : le service worker cachait /read (#90). Confirmé enfin : l'éditeur iPhone affichait ✅ (valeur bien dans le cloud) et l'iPhone se mettait à jour, mais le PC ne suivait pas au rechargement. Cause : le service worker traitait l'API du worker Cloudflare (workers.dev, cross-origin) en CACHE-FIRST — le PC resservait éternellement une ancienne réponse /read en cache et n'adoptait donc jamais le solde modifié ailleurs. La relecture iPhone y échappait uniquement grâce à son URL anti-cache (&_t=). Correctifs : (1) sw.js — l'API workers.dev passe en RÉSEAU SEUL, jamais en cache (cache bumpé v2 pour purger les réponses figées) ; (2) toutes les lectures /read de l'app portent un cache-buster + no-store ; (3) worker v70-cgi — en-tête Cache-Control: no-store sur toutes les réponses. À déployer : app v7.37 (avec le nouveau sw.js) ET worker v70-cgi. Après déploiement, recharger 2× (le temps que le nouveau service worker s'active et purge l'ancien cache)."],
                ["v7.36","Cash matelas — le Dépôt/Retrait ne mettait pas à jour l'affichage/snapshot (#90). Ton édition passait par Dépôt/Retrait, qui ne poussait que cgi_bank — jamais les lignes cgi_portfolio « Cash Matelas » qui alimentent l'affichage ET le snapshot. D'où : snapshot qui ignore le changement, et montant qui repart de l'ancien au reboot. Désormais le Dépôt/Retrait passe par la mécanique robuste (__cgiSetMatelas) : met à jour cgi_bank + les lignes portfolio + le cloud, avec écriture forcée et relecture confirmée. Le bouton « ✎ Modifier le solde » est aussi remonté en HAUT de la fenêtre banque (impossible à rater) pour éditer un solde directement, avec statut ✅/❌ à l'écran. Important : si tes modifs disparaissent après un vidage de cache, c'est qu'elles n'atteignent pas le cloud → l'appareil (souvent l'iPhone) ne joint pas *.workers.dev (4G/DNS/pare-feu). Le bandeau ✅/❌ de l'éditeur est le test réseau définitif."],
                ["v7.35","Cash matelas — éditeur tactile dans la fenêtre banque (#90). Fini la console : clic sur un compte (LCL / BCI / DeBlock) → « Modifier le solde » → saisie → Valider. Utilise la mécanique auto-vérifiée __cgiSetMatelas (lit le savedAt du cloud, écrit au-dessus, relit en anti-cache) et affiche un statut sans ambiguïté : ✅ confirmé dans le cloud / ❌ réseau / ❌ garde-fou (redéployer le worker v70-cgi). Fiable même sans redéploiement du worker. Rappel : pour que les éditions se propagent AUSSI par les chemins normaux (dépôt, snapshot), le worker doit être en v70-cgi (garde-fou par contenu)."],
                ["v7.34","Cash matelas — le garde-fou du worker rejetait les vraies modifs (#90). La console l'a prouvé : réseau OK (cgi_fund_stats passe), mais toute écriture de cgi_bank était ignorée — la relecture renvoyait l'ancien solde. Cause : le worker ne remplaçait cgi_bank que si savedAt entrant > existant ; or le solde bloqué portait un savedAt au niveau/en avance sur l'horloge locale, donc chaque Date.now() perdait le duel (le 1er ✅ était un faux positif : le cloud contenait déjà la valeur d'exemple). Deux correctifs : (1) worker v70-cgi — garde-fou PAR CONTENU : une écriture dont le breakdown DIFFÈRE gagne toujours (indépendant de l'horloge) ; on ne conserve l'existant que si le contenu est identique. (2) __cgiSetMatelas lit d'abord le savedAt du cloud et écrit au-dessus (gagne même sans redéploiement du worker), relit en anti-cache pour confirmer, et logue le duel savedAt. Nécessite de redéployer le worker (v70-cgi) pour que les éditions normales dans l'app se propagent aussi."],
                ["v7.33","Cash matelas — le vrai blocage était le RÉSEAU, plus la logique (#90). La console PC a montré que TOUTES les écritures vers le worker échouaient (ERR_ADDRESS_UNREACHABLE, ERR_NETWORK_CHANGED, timeouts, « Failed to fetch »), et que le service worker ne se mettait pas à jour non plus. Le GET /read du démarrage passait (worker en ligne), mais aucun POST /write-bases n'aboutissait : la valeur saisie sur l'iPhone n'a jamais atteint le cloud (cloud et PC portaient le MÊME ancien solde, d'où « cgi_bank cloud ignoré » — comportement correct puisque rien ne différait). Aucune logique d'adoption ne peut synchroniser une valeur qui ne sort pas de l'appareil : à corriger côté réseau (VPN/pare-feu/filtrage de *.workers.dev). Nouveau : outil manuel AUTO-VÉRIFIÉ __cgiSetMatelas(LCL, BCI, DeBlock) — écrit le solde (local + cloud, horodaté), RELIT le cloud pour confirmer que la valeur y est bien, met à jour l'affichage sans rechargement, et affiche ✅ confirmé / ❌ échec réseau sans ambiguïté. Sert d'outil de secours ET de test réseau définitif."],
                ["v7.32","Cash matelas — LA cause racine enfin trouvée (#90). Le cloud contenait l'ANCIEN solde ({LCL:7159,BCI:838,DeBlock:0}) mais avec un horodatage RÉCENT : un chemin d'écriture non-éditeur re-poussait l'ancienne valeur avec un Date.now() frais, gagnait le garde-fou du worker (exTs>inTs) et écrasait chaque jour l'édition faite sur l'iPhone. Coupable : le snapshot quotidien (doSnapUpload) qui repartait de EFF/CURRENT.bank (le seed) en le tamponnant Date.now(). Correctif : les écritures NON-ÉDITRICES de cgi_bank (snapshot, persistance post-trade) passent par bankSnapshotSafe() — solde LOCAL (= valeur cloud adoptée) avec savedAt PRÉSERVÉ, jamais Date.now(), jamais le seed. Seuls l'observateur de solde et le dépôt/retrait tamponnent Date.now(). Le solde local n'est donc plus jamais écrasé par un snapshot ; une édition sur l'iPhone survit et se propage au PC. À faire une fois après déploiement : ré-saisir le bon solde sur un appareil pour republier la vraie valeur (le cloud porte encore l'ancienne). Rappel PLTR : la vente d'un jour n'apparaît dans le relevé Flex « Open Positions » qu'à J+1 ; saisir la vente à la main est sans risque (réconciliation idempotente, pas de doublon)."],
                ["v7.31","Cash matelas — le garde temporel qui bloquait tout. La réconciliation ne s'appliquait que si le solde du cloud était plus récent que le local ; or l'observateur re-tamponnait le solde local du PC à chaque rafraîchissement, le rendant systématiquement « plus récent » que le cloud → la réconciliation s'auto-annulait. Puisque toute modification locale est publiée au cloud immédiatement, le cloud fait autorité : la réconciliation se déclenche désormais dès que le CONTENU du solde diffère (signature), sans condition d'horodatage, avec anti-boucle par signature. Le solde modifié sur le téléphone est enfin repris sur le PC."],
