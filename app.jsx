...
function gdbFundSeries(anchors, cut){
  try{
    const start=new Date(cut+"T00:00:00").getTime(); const end=Date.now();
    if(!(end>start)) return [];
    const N=24, raw=[];
...
function PageAllocation({hidden, EFF, eur=false, setEur, iconDbVersion=0, bumpIconDb, liveOk=false, onTrade}){
...
═══════════════════════════════════════════════════════════ */
function GdbCompareChartGDB({tf:tfProp, onTFChange, liveGSB, liveGDBS, liveBench, liveGC}){
...
{chartData,hidden,EFF,eur,liveGSB,liveGDBS,liveBench,liveGC,liveDD,liveInv}){
  ...
  const d30  = TF["1M"];
  const dytd = TF["YTD"];
  // #70 — ancrages base-100 des fonds (pour le P&L unique + sparkline au timeframe de la page)
  const _cgicAnchors = gdbFundAnchors(typeof CRYPTO_MONTHLY!=="undefined"?CRYPTO_MONTHLY:{});
  const _cgisAnchors = gdbFundAnchors(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:{});
  // #70 fix — sur les fenêtres courtes (1W/1M/MTD), les données MENSUELLES s'arrêtent au dernier
  // mois clôturé → courbe plate. On privilégie la série QUOTIDIENNE _GDBS (col 2 = CGIC, col 1 = CGIS),
  // accumulée chaque jour par le cron, et on retombe sur l'interpolation mensuelle si pas assez de points.
  const _fundDaily = (col, cut) => {
    const rows=(_GDBS||[]).filter(r=>r&&r[0]>=cut&&r[col]!=null&&r[col]>0);
    if(rows.length>=2){ const f=rows[0][col]; if(f>0) return rows.map(r=>r[col]/f*100); }
    return null;
  };
  const _seriesPerf = ser => { const a=(ser||[]).filter(v=>v!=null); return a.length>=2 ? (a[a.length-1]/a[0]-1) : null; };
  const _cgicSpark = _fundDaily(2, TF[benchTF]) || gdbFundSeries(_cgicAnchors, TF[benchTF]);
  const _cgisSpark = _fundDaily(1, TF[benchTF]) || gdbFundSeries(_cgisAnchors, TF[benchTF]);
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:8,marginBottom:4}}>
        <FondCard label="CGIC — Crypto" cours={gcNAV} qty={_ncgic.sh} fonds={gcFonds} color={C.btc} hidden={hidden}
          ...
          tfLabel={benchTF} perf={_seriesPerf(_cgicSpark)} spark={_cgicSpark}/>
        <FondCard label="CGIS — Actions" cours={gsNAV} qty={_ncgis.sh} fonds={gsFonds} color={C.blue} hidden={hidden}
          ...
          tfLabel={benchTF} perf={_seriesPerf(_cgisSpark)} spark={_cgisSpark}/>
      </div>
      ...
      <GdbCompareChartGDB tf={benchTF} onTFChange={setBenchTF} liveGSB={liveGSB} liveGDBS={liveGDBS} liveBench={liveBench} liveGC={liveGC}/>
  ...
  return(
    <div key={themeName} style={{fontFamily:C.font||"'-apple-system',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,maxWidth:430,margin:"0 auto",paddingBottom:78,boxShadow:themeName==="midnight"?"0 0 80px rgba(180,100,240,.08)":themeName==="bitcoin"?"0 0 80px rgba(247,147,26,.06)":"none"}}>
      ...
      <div style={{padding:"0 16px"}}>
        ...
        {tab===1 && <PageAllocation hidden={hidden} EFF={EFF} eur={eur} setEur={setEur} iconDbVersion={iconDbVersion} bumpIconDb={bumpIconDb} liveOk={liveOk} onTrade={()=>setShowTrade(true)}/>}
        ...
        <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:C.bg1,borderRadius:"20px 20px 0 0",padding:"20px 20px 36px",width:"100%",maxWidth:430,maxHeight:"85vh",overflowY:"auto",border:`1px solid ${C.border}`}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:22}}>
              {[
                ["Groupe D — Sécurité & PIN", [["#62","PIN serveur + code oublié"],["#65","PIN en fin de chargement + à chaque rechargement"],["#66","Mode démo / simulation sur l'écran PIN"],["#55","En attente"]]],
                ...
                ["Groupe J — Bourses & marchés", [["#72","Bourse de Chicago + fenêtre infos par place"]]],
