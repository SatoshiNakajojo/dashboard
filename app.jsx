...
// #90 — Réglage MANUEL et AUTO-VÉRIFIÉ du Cash Matelas. Écrit le solde (local + cloud, horodaté frais),
// PUIS relit /read pour CONFIRMER que le cloud contient bien la valeur, et rafraîchit l'affichage sans
// rechargement. Sert d'outil de secours ET de test réseau sans ambiguïté :
...
async function __cgiSetMatelas(lcl, bci, deblock){
  try{
    ...
    // 2. écrire en LOCAL tout de suite (jamais bloquant) + réaligner les lignes "Cash Matelas" du portfolio
    try{ lsv9Set('cgi_bank', newBank); }catch(e){}
    var patchItems=function(key){ try{ var g=lsv9Get(key); if(!g||!g.items) return null;
      var eU=1/((g.usdEur)|| (function(){ try{ var bk=lsv9Get('cgi_bank'); return 0.92; }catch(e){ return 0.92; } })() || 0.92);
    ...
    // 3. rafraichir l affichage sans reload si le hook est branché
    try{ if(typeof window!=="undefined" && typeof window.__cgiApplyBankLive==="function") window.__cgiApplyBankLive(bd, total); }catch(e){}

    // 4. POUSSER au cloud (batch, timeout long, retries) — cgi_bank + les lignes Cash Matelas du portfolio
    ...
    if(!pushed){ try{ lsv9MarkDirty&&lsv9MarkDirty('cgi_bank'); }catch(e){}
      console.error("[matelas] ❌ ÉCHEC d'écriture au cloud ("+lastErr+"). Le solde est gardé en LOCAL mais N'A PAS atteint le cloud → problème réseau (VPN / pare-feu / workers.dev bloqué ?). Réessaie sur une autre connexion.");
      return {ok:false, reason:"push", detail:lastErr, breakdown:bd}; }

    // 5. RELIRE le cloud pour CONFIRMER que la valeur y est bien
    var confirmed=false, cloudBd=null;
    try{
      var rr=await fetch(CF_WORKER_URL+"/read?k="+encodeURIComponent(CF_AUTH_KEY), { headers:{"X-Auth-Key":CF_AUTH_KEY}, signal:AbortSignal.timeout(25000) });
      if(rr.ok){ var kv=await rr.json(); cloudBd=(kv.cgi_bank&&kv.cgi_bank.breakdown)||null;
        if(cloudBd){ confirmed=Object.keys(bd).every(function(k){ return Math.round(cloudBd[k]||0)===Math.round(bd[k]||0); }); } }
    }catch(e){ lastErr=(e&&e.message)||String(e); }
    if(confirmed){
      console.info("[matelas] ✅ CONFIRMÉ dans le cloud : "+JSON.stringify(bd)+" (total "+total+" €). Recharge l'AUTRE appareil pour qu'il l'adopte.");
      return {ok:true, confirmed:true, breakdown:bd, totalEUR:total};
    }
    console.warn("[matelas] ⚠️ Écriture envoyée mais le cloud n'a PAS confirmé la même valeur (relecture : "+JSON.stringify(cloudBd)+", erreur : "+lastErr+"). À revérifier via /read.");
    return {ok:true, confirmed:false, breakdown:bd, cloudBreakdown:cloudBd};
