  ...
  const handleRefresh = useCallback(async()=>{
    ...
    try{
      ...
      setLive(prev2=>{
        ...
        // #fund-pl — publier NAV + P&L exacts des fonds (mêmes entrées que l'app : calcGdbPrices + fundPeriodPerf)
        try{
          if(prev2 && prev2.crypto && prev2.stocks){
            const _ue3 = prev2.usdEur||CURRENT.usdEur;
            const _gp3 = calcGdbPrices(prev2);
            const cVal = (_gp3.gdbCfondsUSD!=null?_gp3.gdbCfondsUSD:0)*_ue3;
            const sVal = (_gp3.gdbSfondsUSD!=null?_gp3.gdbSfondsUSD:0)*_ue3;
            const cmF=(typeof CRYPTO_MONTHLY!=="undefined"?CRYPTO_MONTHLY:null), smF=(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:null);
            let pC=fundPeriodPerf("CGIC","2022-01-01",cmF,smF,cVal), pS=fundPeriodPerf("CGIS","2022-01-01",cmF,smF,sVal);
            saveBase('cgi_fund_stats',{
              cgic:{nav:_gp3.gdbC,valueEUR:Math.round(cVal),pnlPct:(pC!=null?pC*100:null)},
              cgis:{nav:_gp3.gdbS,valueEUR:Math.round(sVal),pnlPct:(pS!=null?pS*100:null)},
              ts:Date.now()
            });
          }
        }catch(e){}
