...
function cgiHomeSeries(EFF, opts){
  ...
  function build(monthly, field, liveEUR){
    const m = monthly.slice();
    const lastD = m.length ? m[m.length-1].d : "0000";
    journal.forEach(s=>{ if(s&&s.d&&s.d>lastD&&s.d<=today&&s[field]>0) m.push({d:s.d, v:Math.round(s[field])}); });
    if(liveEUR>0){ const k=m.findIndex(x=>x.d===today); if(k>=0) m[k]={d:today,v:liveEUR}; else m.push({d:today,v:liveEUR}); }
    m.sort((a,b)=> a.d<b.d ? -1 : (a.d>b.d?1:0));
    return m;
  }
  const cryptoS = build(monthlyVals(CM), "c", liveC);
  const stocksS = build(monthlyVals(SM), "s", liveS);
  const cMap={}, sMap={};
  ...
  const available = rows.filter(r=>r.total.ok && r.crypto.ok && r.stocks.ok).map(r=>r.tf);
