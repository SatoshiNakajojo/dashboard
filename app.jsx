  ...
  const handleRefresh = useCallback(async()=>{
    ...
    try{
      ...
      setLive(prev2=>{
        ...
        // #fund-pl — publier NAV + P&L exacts des fonds pour le baromètre Telegram (cgi_fund_stats)
        try{
          if(prev2 && prev2.crypto && prev2.stocks){
            const ue2 = prev2.usdEur||CURRENT.usdEur;
            const cE = Math.round((prev2.crypto.total||0)*ue2), sE = Math.round((prev2.stocks.total||0)*ue2);
            const cmF=(typeof CRYPTO_MONTHLY!=="undefined"?CRYPTO_MONTHLY:null), smF=(typeof STOCKS_MONTHLY!=="undefined"?STOCKS_MONTHLY:null);
            let pC=fundPeriodPerf("CGIC","2022-01-01",cmF,smF,cE), pS=fundPeriodPerf("CGIS","2022-01-01",cmF,smF,sE);
            saveBase('cgi_fund_stats',{
              cgic:{nav:prev2.gdbC,valueUSD:Math.round(prev2.crypto.total||0),valueEUR:cE,pnlPct:(pC!=null?pC*100:null)},
              cgis:{nav:prev2.gdbS,valueUSD:Math.round(prev2.stocks.total||0),valueEUR:sE,pnlPct:(pS!=null?pS*100:null)},
              ts:Date.now()
            });
          }
        }catch(e){}
