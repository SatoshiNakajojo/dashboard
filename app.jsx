...
function PageOverview({chartData,onSnapshot,eur,setEur,hidden,setHidden,EFF,refreshing,handleRefresh,refreshedAt,refreshErr,fromSnapshot,gistSync,liveDD,liveCM,liveGDBS,liveGC,chosenSource,iconDbVersion=0,bumpIconDb}){
  ...
  const _fundSparkVals = (fond) => {
    const A = fond==="CGIC"?_cmAnch:_smAnch;
      ...
      <div style={{padding:"22px 20px 22px",borderBottom:`1px solid ${C.border}`}}>
        ...
        {_heroPts && (
          <svg width="100%" height="64" viewBox="0 0 120 34" preserveAspectRatio="none" style={{display:"block",overflow:"visible",marginTop:16}}>
            <polyline points={_heroPts} fill="none" stroke={_dUp?C.green:C.red} strokeWidth="1.5" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        )}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:16,gap:10}}>
          ...
          <span style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:12,color:_dUp?C.green:C.red,fontVariantNumeric:"tabular-nums",border:`1px solid ${(_dUp?C.green:C.red)}47`,borderRadius:999,padding:"4px 11px",whiteSpace:"nowrap"}}>
...
function PageAllocation({hidden, EFF, eur=false, setEur, iconDbVersion=0, bumpIconDb}){
  ...
  // Modifier une cible → redistribue la différence proportionnellement sur les autres (somme = 100)
  const setOneTarget = (name,raw) => {
    ...
    const oldOthers = others.reduce((s,n)=>s+tgtOf(n),0);
    const remain = 100 - v;
    const next = {...allocTargets, [name]:Math.round(v*10)/10};
    if(others.length){
      if(oldOthers>0) others.forEach(n=>{ next[n]=Math.round(tgtOf(n)/oldOthers*remain*10)/10; });
      else others.forEach(n=>{ next[n]=Math.round(remain/others.length*10)/10; });
    }
