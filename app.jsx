...
function gdbAppendLive(anchors, jField, kvCol, liveIdx){
  try{
    ...
    let pts=[];
    // 1. journal cgi_daily (€) — source continue
    try{
      const j=(typeof lsv9Get==="function")?lsv9Get("cgi_daily"):null;
      if(Array.isArray(j)) pts=j.filter(x=>x&&x.d>last.d&&x.d<=today&&x[jField]!=null&&x[jField]>0).map(x=>[x.d,x[jField]]);
    }catch(e){}
    // 2. repli : forme du KV de juin embarqué
    if(pts.length<2){
      const KV=(typeof KV_JUNE!=="undefined"?KV_JUNE:[]);
      const kv=KV.filter(r=>r&&r[0]>last.d&&r[0]<=today&&r[kvCol]!=null&&r[kvCol]>0).map(r=>[r[0],r[kvCol]]);
      if(kv.length>=2) pts=kv;
    }
    if(pts.length<1){ if(last.d<today) anchors.push({d:today, v:target}); return anchors; }
    const n=pts.length, v0=pts[0][1], vN=pts[n-1][1], provN=last.v*(vN/v0), k=(provN>0)?(target/provN):1;
    pts.forEach((p,i)=>{ const prog=(n>1)?i/(n-1):1; anchors.push({d:p[0], v: last.v*(p[1]/v0)*Math.pow(k,prog)}); });
    if(pts[n-1][0]<today) anchors.push({d:today, v:target});
    return anchors;
