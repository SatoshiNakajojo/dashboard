...
const FAMILY_ICONS = {
  "Crypto":"coin", "Indices ETF":"layers", "Stock Picking":"target", "Stocks":"target",
...
{txns, liveFutures, hidden, eur, EFF, liveIbkrAnnex, ibkrTrades=[]}){
  ...
  // #72 — archive des exécutions IBKR : à jour automatiquement à chaque synchro (cgi_ibkr_trades)
  const ibkrList = React.useMemo(function(){
    return (ibkrTrades||[]).filter(function(x){return x&&x.symbol;}).map(function(x){
      return {ticker:(x.symbol||"").split(" ")[0].toUpperCase(), side:(x.side||"").toUpperCase(),
        date:x.date, qty:x.qty, price:x.price, currency:x.currency||"USD"};
    }).sort(function(a,b){ return (a.date||"")<(b.date||"")?1:-1; });
  }, [ibkrTrades]);
  const list = board==="spot" ? spot : board==="futures" ? fut : [];
  ...
  return (
    <div style={{padding:"8px 14px 96px"}}>
      <PageTitle title="History" sub={board==="ibkr" ? "Exécutions IBKR · archive automatique" : ("Trades clôturés · "+(board==="spot"?"Spot":"Futures"))}/>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        ...
        <Tab label="IBKR" active={board==="ibkr"} onClick={function(){setBoard("ibkr");}}/>
      </div>
      {/* Stats globales */}
      {board!=="ibkr" && <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
      ...
      {/* Tri */}
      {board!=="ibkr" && <div style={{display:"flex",gap:8,marginBottom:10,alignItems:"center"}}>
      ...
      {/* #72 — Onglet IBKR : exécutions archivées, informationnel (hors positions/P&L, anti-doublon) */}
      {board==="ibkr" && (
        <div>
          <div style={{fontSize:11,color:C.text3,marginBottom:10}}>{ibkrList.length} exécution(s) — mises à jour automatiquement à chaque synchronisation IBKR. Informationnel : hors calculs de positions et de P&L.</div>
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {ibkrList.map(function(t,i){ return (
              <div key={i} style={{display:"flex",alignItems:"center",gap:8,fontSize:11,borderBottom:`1px solid ${C.border}`,paddingBottom:7}}>
                <span style={{fontSize:9,fontWeight:700,color:t.side==="BUY"?C.green:C.red,border:`1px solid ${(t.side==="BUY"?C.green:C.red)}55`,borderRadius:5,padding:"1px 6px",minWidth:42,textAlign:"center"}}>{t.side==="BUY"?"ACHAT":"VENTE"}</span>
                <span style={{fontSize:8,fontWeight:700,color:C.gold,border:`1px solid ${C.gold}55`,borderRadius:5,padding:"1px 5px"}}>IBKR</span>
                <span style={{color:C.text3,minWidth:62}}>{t.date}</span>
                <span style={{fontWeight:600,color:C.text,minWidth:52}}>{t.ticker}</span>
                <span style={{flex:1,textAlign:"right",color:C.text}}>{msk(Math.abs(t.qty||0).toLocaleString("fr-FR",{maximumFractionDigits:4})+" @ "+(t.currency==="EUR"?"€":"$")+Number(t.price||0).toFixed(2),hidden)}</span>
              </div>); })}
            {ibkrList.length===0 && <div style={{textAlign:"center",color:C.text3,fontSize:13,padding:30}}>Aucune exécution archivée — lancer une synchronisation IBKR.</div>}
          </div>
        </div>
      )}
    ...
    <div style={{position:"fixed",inset:0,zIndex:99999,background:ONYX,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'-apple-system',sans-serif",color:"#F5F0E8",padding:24,boxSizing:"border-box"}}>
      ...
      {onDemo && (
        <button onClick={onDemo} style={{
          marginTop:34,background:"transparent",border:`1px solid rgba(201,168,106,.3)`,
          borderRadius:999,padding:"9px 22px",color:T2,fontSize:11,letterSpacing:1.8,
          textTransform:"uppercase",fontFamily:"'-apple-system',sans-serif",cursor:"pointer",
          WebkitTapHighlightColor:"transparent"
        }}>Mode démo · montants masqués</button>
      )}
    </div>
  );
}
...
function App(){
  ...
  const[demo,setDemo]=useState(false);   // #66 — mode démo (montants masqués, lecture seule)
    ...
    (async()=>{
      ...
      setLiveGDBS(cleanedGDBS);
          ...
          try{ const _it=kvData.cgi_ibkr_trades; if(_it && Array.isArray(_it.trades)){ setIbkrTrades(_it.trades);
            console.info("[ibkr] archive exécutions chargée : "+_it.trades.length+" (visibles dans History via la recherche par ticker)"); } }catch(e){}
  ...
  if(!unlocked) return <PinLock onUnlock={()=>{ setDemo(false); setUnlocked(true); }} onDemo={()=>{ setDemo(true); setHidden(true); setUnlocked(true); }}/>;
  ...
  return(
    <div key={themeName} style={{fontFamily:C.font||"'-apple-system',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,maxWidth:430,margin:"0 auto",paddingBottom:78,boxShadow:themeName==="midnight"?"0 0 80px rgba(180,100,240,.08)":themeName==="bitcoin"?"0 0 80px rgba(247,147,26,.06)":"none"}}>
      {demo && (
        <div onClick={()=>{ setDemo(false); setHidden(false); setUnlocked(false); }} style={{position:"fixed",top:8,left:"50%",transform:"translateX(-50%)",zIndex:99998,background:"rgba(201,168,106,.15)",border:"1px solid rgba(201,168,106,.5)",color:C.gold,fontSize:10,letterSpacing:2,textTransform:"uppercase",padding:"4px 12px",borderRadius:999,cursor:"pointer",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)"}}>Mode démo · toucher pour quitter</div>
      )}
      <div style={{
