...
function PageOverview({chartData,onSnapshot,eur,setEur,hidden,setHidden,EFF,refreshing,handleRefresh,refreshedAt,refreshErr,fromSnapshot,gistSync,liveDD,liveCM,liveGDBS,liveGC,chosenSource,iconDbVersion=0,bumpIconDb}){
  ...
  const _fundSparkVals = (fond, valEUR) => {
    const today = _po_today;
    const A = (fond==="CGIC"?_cmAll:_smAll).filter(p=>p.d<=today); // mensuel + quotidien (journal #16) jusqu'a aujourd'hui
    const win = A.filter(p=>p.d>=_hcut);
    // si la fenetre a <2 points (peu de jours dans le journal) : montre les 2 derniers points connus
  ...
  const _mkPerf = (name,color,valUSD,inv) => {
    ...
    if(pct==null){ // #16 — court terme : today vs 1er point du journal dans la fenetre
      const fld = fond==="CGIC"?"c":"s";
      const w = _daySnaps.filter(x=>x && x.d>=_hcut && x[fld]>0);
      if(w.length>=1 && w[0][fld]>0) pct = valEUR/w[0][fld] - 1;
    }
    if(pct==null){ const lastMo=_fundLastMonthEUR(_MAP_OF(fond)); pct = (lastMo>0) ? (valEUR/lastMo - 1) : (inv>0 ? valEUR/inv - 1 : 0); }   // sinon : live vs derniere cloture, ou depuis creation
    const pnlEUR = Math.round(valEUR - valEUR/(1+pct));   // gain sur la période
    const pnl = eur ? pnlEUR : Math.round(pnlEUR/_ue);
    const up  = pct>=0;
    const sv = _fundSparkVals(fond, valEUR);
  ...
  // #53 — delta du patrimoine COHÉRENT avec les perfs des fonds (pondéré par valeur ; cash/banque ~0%)
  const _blendPct = (()=>{ const cv=_cryptoUSD||0, sv=_stocksUSD||0, base=(_sumUSD||(cv+sv))||1;
    const cp=_perf[0]?(_perf[0].pct||0):0, sp=_perf[1]?(_perf[1].pct||0):0; return (cv*cp+sv*sp)/base; })();
  const _bUp = _blendPct>=0;
  const _blendCur = eur ? _sumEUR : _sumUSD;
  const _blendPnl = Math.round(_blendCur - _blendCur/(1+_blendPct));
  // #53 — courbe du patrimoine = indice mélangé des fonds (pondéré), cohérent avec le delta
  const _blendHeroPts = (()=>{
    const today=_po_today;
    const cv=_cryptoUSD||0, sv=_stocksUSD||0, tot=(cv+sv)||1;
    const cMap={}, sMap={};
    _cmAll.forEach(p=>{ if(p.d<=today) cMap[p.d]=p.v; });
    _smAll.forEach(p=>{ if(p.d<=today) sMap[p.d]=p.v; });
    let dates=Object.keys(cMap).filter(d=>sMap[d]!=null).sort();
    let win=dates.filter(d=>d>=_hcut);
    if(win.length<2) win=dates.slice(-2);
    const vals=win.map(d=>(cv/tot)*cMap[d] + (sv/tot)*sMap[d]);
          ...
          <div style={{position:"relative"}}>
            ...
            {heroTFOpen && (
              <div style={{position:"absolute",top:"100%",left:0,marginTop:6,zIndex:30,background:C.bg2,border:`1px solid ${C.border2}`,borderRadius:C.radiusSm||10,padding:5,minWidth:150,boxShadow:"0 10px 28px rgba(0,0,0,0.45)"}}>
                {["D","3D","1S","1M","3M","YTD","1A","ALL"].map(tf=>(
