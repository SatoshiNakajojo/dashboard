              ...
              if(kvBank && kvBank.savedAt){
                ...
                if(kvBank.savedAt>_lbTs){ lsv9Set('cgi_bank', kvBank);
                  ...
                  try{
                    ...
                    const _syncBankItems=(key)=>{ const _p=lsv9Get(key); if(!_p||!_p.items) return;
                      let _chg=false; const eU=(_p.usdEur|| (window.__CGI_UE)||0.92);
                      _p.items=_p.items.map(it=>{ if((it.cat)!=="Cash Matelas") return it;
                        const kk=Object.keys(_bd).find(k=>k.toUpperCase()===(it.t||"").toUpperCase());
                        if(kk==null) return it; const v=_bd[kk]||0;
                        if((it.valEUR||it.qty)!==v){ _chg=true; return {...it, valEUR:v, qty:v, val:Math.round(v*(1/(eU||0.92)))}; }
                        return it; });
                      if(_chg){ lsv9Set(key,_p); }
                    };
                    _syncBankItems('cgi_portfolio'); _syncBankItems('cgi_stocks');
                    console.info("[bank] items Cash Matelas du portfolio réalignés sur le breakdown cloud");
                  }catch(e){}
